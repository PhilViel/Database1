...
Note                :	ADX0000863	IA	2006-03-31	Bruno Lapointe		Création
			ADX0000984	IA	2006-05-15	Alain Quirion		Modification		Ajout de l'objet Un_TFR
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[SL_UN_OperTFR] (
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			-- Va chercher les données des opérations TFR s'il y en a (Un_TFR)
			IF @iResult > 0
				EXECUTE SL_UN_WriteTFRInBlob @OperID, @iBlobID, @pBlob, @vcBlob OUTPUT, @iResult OUTPUT

			-- Va chercher les données des frais disponibles s'il y en a (Un_AvaibleFeeUse)
			IF @iResult > 0
				EXECUTE SL_UN_WriteAvailableFeeUseInBlob @OperID, @iBlobID, @pBlob, @vcBlob OUTPUT, @iResult OUTPUT
