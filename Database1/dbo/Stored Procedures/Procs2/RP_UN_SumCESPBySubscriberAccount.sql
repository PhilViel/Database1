...
Note                :	ADX0001170	IA	2006-11-14	Bruno Lapointe			Création
						ADX0002426	BR	2007-05-22	Alain Quirion			Modification : Un_CESP au lieu de Un_CESP900
										...
										2013-08-12	Donald Huppé			glpi 9898 : ajout année signature et filtre sur l'état de convention (@cConventionStateIDs)
										2014-12-16	Donald Huppé			Ne plus faire primary key dans #tOperTIO
										2016-05-16	Donald Huppé			JIRA TI-2317 : Ajout du PRA
                                        2018-11-08  Pierre-Luc Simard       Utilisation du nom du plan complet
																			
exec RP_UN_SumCESPBySubscriberAccount 0, '2011-01-01', '2011-02-24', 0, '1096661', 2011, 0
exec RP_UN_SumCESPBySubscriberAccount 0, '2016-02-26', '2016-02-26', 17, NULL, NULL, 0, ''

*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[RP_UN_SumCESPBySubscriberAccount] (	
	...
	@iColumnFilter INTEGER ,--	Filtre sur les colonnes associées à des comptes G/L du rapport :
									...
									--		13 = Subventions transférées (ARI)
	...
	@cConventionStateIDs varchar(100) = ''
	)
AS
BEGIN
	...
	INSERT INTO #tConventionState
		...
		FROM ( -- Retourne le plus grand ID pour la plus grande date de début d'un état par convention
			...
			FROM (-- Retourne la plus grande date de début d'un état par convention
				...
				WHERE 
					LEFT(CONVERT(VARCHAR, S.StartDate, 120), 10) <= @dtEnd -- État à la date de fin de la période
		...
		*/
		where 
			@cConventionStateIDs = '' 
			OR (@cConventionStateIDs <> '' and CHARINDEX (CCS.ConventionStateID,@cConventionStateIDs) > 0)
			...
			FROM (
				...
				WHERE CO.ConventionOperTypeID IN ('IBC', 'INS', 'IS+', 'IST')
					AND CO.OperID IN (
						SELECT OperID
						FROM Un_Oper
						WHERE OperTypeID = 'PRA' -- Opérations PAE
						AND (
							(@bAll = 1 and LEFT(CONVERT(VARCHAR, OperDate, 120), 10) <= @dtEnd)
							OR
							(@bAll = 0 AND LEFT(CONVERT(VARCHAR, OperDate, 120), 10) BETWEEN @dtStart AND @dtEnd)
	...
	SELECT
		...
		fCESPInt_PRA = ISNULL(CO.fCESPInt_PRA,0),-- Intérêts SCEE, SCEE+ et BEC (PRA)
		...
		fCESG_ACESG_CLB_ARI = ISNULL(CESP.fCESG_ACESG_CLB_ARI,0), -- Subvention transférés (ARI)
		SoldeNetGarantie =	
			ISNULL(CESP.fCESG,0) + 
			...
			ISNULL(CESP.fCESG_ACESG_CLB_ARI,0),
		SoldeNetRendement = 
			ISNULL(CO.fINS,0) + 
	...
	LEFT JOIN ( -- GLPI 3157 faire un LEFT JOIN plutot qu'un JOIN
		SELECT
			...
			AnneeSignature = MIN(YEAR(U.signatureDate))
		FROM #tConventionState CS
	...
	WHERE ( CESP.ConventionID IS NOT NULL
		...
		AND(	@iColumnFilter = 0	-- 0 = Une des colonnes (pas de filtre)
			...
			OR ( @iColumnFilter = 12 -- 12 = Rendements transférés (ARI)
				AND ISNULL(CO.fINS_ARI,0) <> 0
				)
			OR ( @iColumnFilter = 13 -- 13 = Subvention transférés (ARI)
				AND ISNULL(CESP.fCESG_ACESG_CLB_ARI,0) <> 0
				)
				
			OR ( @iColumnFilter = 14 -- 14 = Subvention ou rendements (RIO)
				AND (ISNULL(CESP.fCESG_ACESG_CLB_RIO,0) <> 0  OR ISNULL(CO.fCESPInt_RIO,0) <> 0)
				)
				
			OR ( @iColumnFilter = 15 -- 15 = Subvention ou rendement (TRI)
				AND (ISNULL(CESP.fCESG_ACESG_CLB_TRI,0) <> 0  OR ISNULL(CO.fCESPInt_TRI,0) <> 0)
				)

			OR ( @iColumnFilter = 16 -- 16 = Subvention ou rendement (RIM)
				AND (ISNULL(CESP.fCESG_ACESG_CLB_RIM,0) <> 0  OR ISNULL(CO.fCESPInt_RIM,0) <> 0)
				)

			OR ( @iColumnFilter = 17 -- 17 = Subvention ou rendement (PRA)
				AND (ISNULL(CO.fCESPInt_PRA,0) <> 0 )
				)				
			)
	ORDER BY
