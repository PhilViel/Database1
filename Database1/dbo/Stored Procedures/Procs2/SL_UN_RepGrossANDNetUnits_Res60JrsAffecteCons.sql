...
Note                :	2014-04-02	Donald Huppé	    Créaton comme SL_UN_RepGrossANDNetUnits mais rasion résiliation "60 jours - 2014" affecte le taux de conservation
													    Pour un rapport de vente
													    glpi 10687
						2014-06-04	Donald Huppé	    suivi glpi 10687 : valider que la résil est dans un délai de 60 jours

						2015-04-08	Donald Huppé	    Modifier le calcul des résiliations et réinscriptions : on calcule la qté d'unité selon le ratio des frais encaissés p/r 
													    au frais total prévu : (M.FeeByUnit - UR.FeeSumByUnit) / M.FeeByUnit

						2015-04-08	Donald Huppé	    Faire un Merge des Rep Corpo vers le Rep Original (voir table tblREPR_Lien_Rep_RepCorpo)
						2015-04-08	Donald Huppé	    correction suite au merge des rep corpo : valider le businessStart du rep original pour détermimer s'il est recrue (voir calcul de recrue)
                        2018-11-08  Pierre-Luc Simard   Utilisation du regroupement de régime

exec SL_UN_RepGrossANDNetUnits NULL, '2014-01-01','2014-12-31', 0, 1
EXEC SL_UN_RepGrossANDNetUnits 490, NULL, NULL, 0, 0

exec SL_UN_RepGrossANDNetUnits NULL, '2014-09-01', '2014-10-14', 0, 0

****************************************************************************************************/
CREATE PROCEDURE [dbo].[SL_UN_RepGrossANDNetUnits_Res60JrsAffecteCons] 
	(
...
*/
 ...
 --PRINT '3 - ' + CAST(DATEDIFF(MILLISECOND, @dtBegin, GETDATE())/1000 AS VARCHAR(20))
	...
	INSERT INTO #tReinscriptions
		SELECT 
			...
			Recrue = case when DATEADD(mm,12, isnull(rc.BusinessStart, R.BusinessStart)) > U1.dtFirstDeposit then 1 else 0 end--dbo.fn_Un_IsRecruit(R.BusinessStart, U1.dtFirstDeposit)
		FROM Un_AvailableFeeUse A
		JOIN Un_Oper O ON O.OperID = A.OperID
		JOIN Un_Cotisation C ON C.OperID = O.OperID
		JOIN dbo.Un_Unit U1 ON U1.UnitID = C.UnitID
		...
		WHERE (U1.dtFirstDeposit > RT.LastRepTreatmentDate AND U1.dtFirstDeposit <= RT.RepTreatmentDate)
			...
			AND (ISNULL(URR.bReduitTauxConservationRep,1) = 1  OR (URR.UnitReductionReasonID = 37 AND DATEDIFF(d,U1.SignatureDate,UR.ReductionDate) <= 60)  ) -- La raison de la résiliation doit être marquée comme affectant le taux de conservation ou encore non-définie
		GROUP BY
			U1.UnitID,
			Uori.unitid,
			RepTreatmentID,
 ...
 --PRINT '4 - ' + CAST(DATEDIFF(MILLISECOND, @dtBegin, GETDATE())/1000 AS VARCHAR(20))
	...
	SELECT 
		U.UnitID,
		U.RepID,
		Recrue = case when DATEADD(mm,12,isnull(rc.BusinessStart, R.BusinessStart)) > U.dtFirstDeposit then 1 else 0 end,-- dbo.fn_Un_IsRecruit(R.BusinessStart, U.dtFirstDeposit),
 ...
 --PRINT '6 - ' + CAST(DATEDIFF(MILLISECOND, @dtBegin, GETDATE())/1000 AS VARCHAR(20))
	...
	SELECT 
		U.UnitID,
		U.RepID,
		Recrue = case when DATEADD(mm,12,isnull(rc.BusinessStart, R.BusinessStart)) > UR.ReductionDate then 1 else 0 end, --dbo.fn_Un_IsRecruit(R.BusinessStart, UR.ReductionDate), 
		Bossid = isnull(MB.BossID,0),
		T.RepTreatmentID,
		UnitQtyRes = 
			SUM(
	...
	WHERE UR.FeeSumByUnit < M.FeeByUnit
	    AND (ISNULL(URR.bReduitTauxConservationRep,1) = 1 OR (URR.UnitReductionReasonID = 37 AND DATEDIFF(d,U.SignatureDate,UR.ReductionDate) <= 60)) -- La raison de la résiliation doit être marquée comme affectant le taux de conservation ou encore non-définie
	GROUP BY 
		U.UnitID,
		U.RepID,
