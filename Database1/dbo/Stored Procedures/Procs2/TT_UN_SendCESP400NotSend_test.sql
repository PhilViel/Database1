...
exec TT_UN_SendCESP400NotSend_test
					
					-- *********** inscrire la convention et l'operid du FDB à reprendre ***************
					
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[TT_UN_SendCESP400NotSend_test]
	(
	@MaConv varchar(20),
	@MonOperid int
	)
...
BEGIN
	...
	INSERT INTO #LockAccount	
		...
		WHERE dtRegStartDate IS NOT NULL
			...
			AND C.ConventionNo = @MaConv --'R-20080428038'

	SELECT * FROM #LockAccount
	...
	INSERT INTO Un_CESP400 (
		...
		WHERE 1=1
			AND O.OperID = @MonOperid --17910024
/*			
			-- L'opération n'a pas été faite dans un compte bloqué
/*			and	( Ct.EffectDate > L.dtRegStartDate
			...
			AND( O.OperTypeID IN ('PRD', 'CHQ', 'CPA', 'FCB')
				OR ( O.OperTypeID = 'TFR'
	...
	INSERT INTO Un_CESP400 (
		...
		WHERE 
			...
			AND ISNULL(IR.ProgramLength,0) > 0
