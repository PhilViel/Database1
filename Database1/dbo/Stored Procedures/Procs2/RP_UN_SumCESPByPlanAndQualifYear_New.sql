...
exec RP_UN_SumCESPByPlanAndQualifYear_NEW 0, '1950-12-01', '2009-12-31', 0
												
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[RP_UN_SumCESPByPlanAndQualifYear_New] (	
...
BEGIN
	...
	--select @vcOPER_RENDEMENT_POSITIF_NEGATIF
...
--	IF @bAll = 1
		...
		select
			...
			GROUP BY
				CE.ConventionID
			/*
			HAVING SUM(CASE WHEN O.OperTypeID = 'SUB' THEN CE.fCESG ELSE 0 END) <> 0 -- Au moins un des montants doit être différent de 0.00$
				OR SUM(CASE WHEN O.OperTypeID = 'SUB' THEN CE.fACESG ELSE 0 END) <> 0
				OR SUM(CASE WHEN O.OperTypeID = 'SUB' THEN CE.fCLB ELSE 0 END) <> 0
				OR SUM(CASE WHEN O.OperTypeID = 'TIN' THEN CE.fCESG + CE.fACESG ELSE 0 END) <> 0
				OR SUM(CASE WHEN O.OperTypeID = 'TIN' THEN CE.fCLB ELSE 0 END) <> 0
				OR SUM(CASE WHEN O.OperTypeID = 'OUT' THEN CE.fCESG + CE.fACESG ELSE 0 END) <> 0
				OR SUM(CASE WHEN O.OperTypeID = 'OUT' THEN CE.fCLB ELSE 0 END) <> 0
				OR SUM(CASE WHEN O.OperTypeID = 'PAE' THEN CE.fCESG + CE.fACESG + CE.fCLB ELSE 0 END) <> 0
				OR SUM(CASE WHEN O.OperTypeID = 'RIO' THEN CE.fCESG + CE.fACESG + CE.fCLB ELSE 0 END) <> 0
			*/
...
/*	ELSE
		...
		INSERT INTO #tCESP
			...
			JOIN Un_Oper O ON O.OperID = CE.OperID
			WHERE O.OperTypeID IN ('OUT', 'TIN', 'PAE', 'SUB', 'RIO')
				AND O.OperDate BETWEEN @dtStart AND @dtEnd -- Opération de la période sélectionnée.
			GROUP BY
				CE.ConventionID
			HAVING SUM(CASE WHEN O.OperTypeID = 'SUB' THEN CE.fCESG ELSE 0 END) <> 0 -- Au moins un des montants doit être différent de 0.00$
				OR SUM(CASE WHEN O.OperTypeID = 'SUB' THEN CE.fACESG ELSE 0 END) <> 0
				...
				WHERE CO.ConventionOperTypeID IN ('IBC', 'INS', 'IS+', 'IST') -- Int. TIN
					AND CO.OperID IN (
						...
						WHERE (CHARINDEX(Un_Oper.OperTypeID,@vcOPER_RENDEMENT_POSITIF_NEGATIF) > 0)
							AND OperDate BETWEEN @dtStart AND @dtEnd -- Dans la période
						)
				---------
				UNION ALL
				---------
