...
Nom                 :	VL_UN_OperCHQ_IU
Description         :	Procédure de validation avant la sauvegarde d’ajout/modification de chèques.
Exemple d'appel		:   EXEC dbo.VL_UN_OperCHQ_IU 2
...
Note                :	ADX0000847	IA	2006-03-16	Bruno Lapointe			Création
						ADX0001200	IA	2006-11-15	Alain Quirion			Ajout de la validation CHQ05
						ADX0002371	BR	2007-04-09	Alain Quirion			Correction du calcul des taxes lors de la validation CHQ05
										...
										2010-11-29	Pierre Paquet			Correction: Utilisation de @ConventionTable
        2015-12-01      Steeve Picard               Utilisation de la nouvelle fonction «fntCONV_ObtenirStatutConventionEnDate_PourTous»
 *********************************************************************************************************************/
CREATE PROCEDURE [dbo].[VL_UN_OperCHQ_IU] (
	@iBlobID INT) -- ID du blob de la table CRI_Blob qui contient les objets de l’opération CHQ à sauvegarder
AS
BEGIN
	DECLARE 
		@iResult INT
...
*/
		...
		IF EXISTS (
				SELECT OperID
				FROM @OperTable
				WHERE OperTypeID = 'CHQ')
		BEGIN
			-- CHQ01 -> Le montant de l’opération doit être plus grand que 0.00$.
			...
			-- CHQ02 -> La date d’opération doit être plus grande que la date de barrure du système
			INSERT INTO #WngAndErr
				SELECT 
					'CHQ02',
					'',
			...
			INTO #Tmp_BeneficiaryCeilingCfgCHQ
			FROM (
				SELECT
					C.BeneficiaryID,
			...
			-- CHQ03 -> Le montant d'épargne et de frais cotisés pour cette année dépasse le maximum annuel
			INSERT INTO #WngAndErr
				SELECT 
					'CHQ03',
				...
				JOIN #Tmp_BeneficiaryCeilingCfgCHQ TBC ON TBC.BeneficiaryID = B.BeneficiaryID
			...
			DROP TABLE #Tmp_BeneficiaryCeilingCfgCHQ

			-- CHQ04 : Les multiples opérations ne sont pas géré pour les CHQ 
