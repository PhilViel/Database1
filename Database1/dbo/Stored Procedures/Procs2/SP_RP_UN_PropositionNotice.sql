...
Nom                 :	SP_RP_UN_PropositionNotice
Description         :	Rapport de fusion word des accusés réceptions de compte transitoire
Valeurs de retours  :	Dataset des données du document.
Note                :	ADX0000???	IA	2004-06-01	Bruno Lapointe		Création
	...
	exec SP_RP_UN_PropositionNotice 1,317449 , 1
	
	select * from un_plan -- select * from mo_lang
	select * from un_convention where conventionno = 'r-20080703006'																			
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[SP_RP_UN_PropositionNotice] (
...
BEGIN
	...
	SELECT 
		@DocTypeID = DocTypeID
	FROM CRQ_DocType
	WHERE DocTypeCode = 'PropositionNotice'
	...
	INSERT INTO #Notice
		SELECT
			...
			*/
			...
			LetterMedDate = dbo.fn_Mo_DateToLongDateStr (@Today, HS.LangID),
	...
	SELECT 
		DocTemplateID,
		LangID,
		ConventionNo = n.ConventionNo /*+ ' (' + case 
		...
		SubscriberID
	FROM #Notice n
	--JOIN dbo.Un_Convention c on n.ConventionNo = c.ConventionNo
	--join un_plan p on c.planid = p.planid
