...
Nom                 :	SL_UN_CESP400And511
Description         :	Retourne l’information d’un enregistrement 400 et 511
Valeurs de retours  :
	Dataset :
		tiCESP400TypeID				    TINYINT			Type d’enregistrement 400 ou 511
...
Note                :	ADX0001122	IA	2006-09-20	Bruno Lapointe		Création
                                        2008-10-20  Fatiha Araar Ajouter le détails de la 511
...
***************************************************************************************************************************/
CREATE PROCEDURE [dbo].[SL_UN_CESP400And511] ( @iCESPID INTEGER,-- ID de l’enregistrement 400 ou 511
                                               @bISCESP400 BIT )--Booleen qui indique si c'est une 400 ou 511) 
AS

BEGIN

IF @bISCESP400 = 1  --On veut le détails d'une transaction 400
 BEGIN
	SELECT
	...
	WHERE C4.iCESP400ID = @iCESPID
END

ELSE --On veut le détails d'une transaction 511

   SELECT
 		tiCESP400TypeID = '12', -- Type d’enregistrement 511
		C5.vcTransID, -- ID PCEE : ID unique de la transaction qui a été envoyé au PCEE.
		C5.dtTransaction, -- Date envoyé au PCEE pour la transaction.
		S.dtCESPSendFile, -- Date d’envoi de la transaction au PCEE
		C5.ConventionNo, -- Numéro de convention envoyé au PCEE. 
		C5.iPlanGovRegNumber, -- Numéro du régime attribué par le gouvernement.
		C4.vcBeneficiarySIN, -- NAS du bénéficiaire au moment de la transaction.
		C4.vcSubscriberSINorEN, -- NAS du souscripteur. Le PCEE sans sert pour des raisons fiscal.
		bCESPDemand = CAST(0 AS BIT), -- Indique au PCEE si le souscripteur a demandé de recevoir de la subvention pour cette transaction.
		fCotisation = 0, -- Montant de cotisation (épargne et frais) que l’on désire subventionner.
		C5.vcPCGFirstName, -- Prénom du principal responsable. Utilisé par le PCEE pour déterminer si le bénéficiaire est éligible à la SCEE+.
		C5.vcPCGLastName, -- Nom du principal responsable. Utilisé par le PCEE pour déterminer si le bénéficiaire est éligible à la SCEE+.
		C5.vcPCGSINorEN, -- NAS du principal responsable s’il s’agit d’une personne, NE s’il s’agit d’une entreprise. Utilisé par le PCEE pour déterminer si le bénéficiaire est éligible à la SCEE+.
		C4.dtStudyStart, -- Date de début des études du bénéficiaire. Fait parti de la preuve d’inscription qui est obligatoire pour le versement de PAE.
		C4.tiStudyYearWeek, -- Nombre de semaines d’études par année à l’établissement d’enseignement du bénéficiaire. Fait parti de la preuve d’inscription qui est obligatoire pour le versement de PAE.
		C4.tiProgramLength, -- Durée du programme : 	Nombre d’année que dure le programme d’études.
		C4.siProgramYear, -- Année du programme	 : Année à laquelle est rendu le bénéficiaire dans le programme d’études.
		C4.cCollegeTypeID, -- Type d’établissement d’enseignement : Université, Collège ou Cégep, Établissement privés, Autres. 
		C4.vcCollegeCode, -- Code postal de l’établissement d’enseignement.
        fEAP = 0, -- Montant du PAE versée : Montant du chèque du PAE.
		fEAPCESG = 0, -- Montant de SCEE et SCEE + inclus dans le montant du chèque du PAE.
		fEAPCLB = 0, -- Montant de BEC inclus dans le montant du chèque du PAE.
		fPSECotisation = 0, -- Montant de cotisation remboursé au souscripteur dans le retrait EPS.
		fCESG =	0, -- Montant de SCEE à rembourser ou transféré.
		fACESGPart = 0, -- Montant de SCEE+ à rembourser ou transféré.
		fCLB =0, -- Montant de BEC à rembourser ou transféré.
		iOtherPlanGovRegNumber= 0, -- Numéro du régime attribué par le gouvernement du régime cédant (19 – Transfert entré) ou du régime cessionnaire (23 – Transfert sortie).
		vcOtherConventionNo= 0, -- Numéro du contrat régime cédant (19 – Transfert entré) ou du régime cessionnaire (23 – Transfert sortie).
		tiCESP400WithdrawReasonID= 0, -- Raison du remboursement : Retrait de cotisations, Paiement de revenu accumulé (PRA), Résiliation de contrat, Transfert inadmissible, Remplacement d'un bénéficiaire inadmissible, Paiement versé à un établissement d'enseignement, Révocation, Ne répond plus au critère frère et sœur, Décès, Retrait de cotisations excédentaires et Autres.
		vcCESP400WithdrawReason = '', -- Raison du remboursement de subvention (400)
		tiCESPStatus =
			CASE
				WHEN C5.iCESP800ID IS NOT NULL THEN 3
			ELSE 0
			END , -- Statut de l’enregistrement 500. (0 = Normal, 3 = En Erreur)
			 
		R.dtRead, -- Date de réception de la réponse.
		iCESP900ID = 0,
		fCESG_900 = 0, -- Montant de SCEE reçue.
		vcCESP900CESGReason = '', -- Raison pour laquelle la SCEE n’a pas été totalement versée. Si elle est vide c’est qu’elle a toute été versée.
		fACESG_900 = 0, -- Montant de SCEE + reçue.
		vcCESP900ACESGReason = '', -- Raison pour laquelle la SCEE+ n’a pas été totalement versée. Si elle est vide c’est qu’elle a toute été versée.
		fCLB_900 = 0, -- Montant de BEC reçu.
		fCLBFee = 0, -- Montant de frais reçu pour la gestion du BEC.
		C8.siCESP800ErrorID, -- Code PCEE de l’erreur (Ex : 7001, 7006, etc.).
		C8.vcErrFieldName, -- Champ pointé par le PCEE comme étant la source de l’erreur.
		C8R.vcCESP800Error, -- Description donnée par le PCEE de l’erreur. Elle pourra être sur plusieurs lignes.
		vcTransID400 = C5.vcOriginalTransID -- ID de la transaction 400 d'origine
	FROM Un_CESP511 C5
    LEFT JOIN Un_CESP400 C4 ON C5.iOriginalCESP400ID = C4.iCESP400ID
	LEFT JOIN Un_CESPSendFile S ON S.iCESPSendFileID = C5.iCESPSendFileID
	LEFT JOIN Un_CESPReceiveFile R ON R.iCESPReceiveFileID = S.iCESPReceiveFileID
	LEFT JOIN Un_CESP800 C8 ON C8.iCESP800ID = C5.iCESP800ID
	LEFT JOIN Un_CESP800Error C8R ON C8R.siCESP800ErrorID = C8.siCESP800ErrorID
	WHERE C5.iCESP511ID = @iCESPID
 END
