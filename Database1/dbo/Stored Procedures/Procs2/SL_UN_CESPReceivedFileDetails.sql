...
*********************************************************************************************************************/
CREATE PROCEDURE dbo.SL_UN_CESPReceivedFileDetails (
...
BEGIN
	...
	SELECT
		...
		vcDescription = O.vcCESP900Origin,
	...
	SELECT
		...
		vcDescription = R.vcCESP900CESGReason,
		fCount = ISNULL(COUNT(C9.iCESP900ID),0),
		dtReceived = NULL
	...
	SELECT
		...
		vcDescription = R.vcCESP900ACESGReason,
		fCount = ISNULL(COUNT(C9.iCESP900ID),0),
		dtReceived = NULL
	...
	SELECT
		...
		dtReceived = NULL
	FROM Un_CESP900 C9
	WHERE C9.iCESPReceiveFileID = @iCESPReceiveFileID) V
	ORDER BY
		1,
		2
END
