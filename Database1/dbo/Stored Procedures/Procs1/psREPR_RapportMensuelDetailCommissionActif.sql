...
Nom                 :	psREPR_RapportDetailCommissionActif
Description         :	Pour le rapport SSRS "RapportMensuelDetailCommissionActif" : permet d'obtenir les commissions sur l'actif
						pour un représentant en particulier pour un mois choisi.
Valeurs de retours  :	Dataset 
Note                :	2016-05-20	Maxime Martel			Création

exec psREPR_RapportMensuelDetailCommissionActif '2016-06-01', 149509
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[psREPR_RapportMensuelDetailCommissionActif] 
	(
	@StartDate DATETIME, 
	@RepID INTEGER
	) 
...
BEGIN
	...
	FROM 
		...
		LEFT JOIN VtblREPR_CommissionsSurActif_Conv CSA on CSA.RepID = R.RepID AND MONTH(@dtdateCalcul) = MONTH(CSA.dDate_Calcul) AND YEAR(@dtDateCalcul) = YEAR(CSA.dDate_Calcul)
