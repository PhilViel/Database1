...
Code de service		:		psOPER_ObtenirLettreConfirmationRIM
Nom du service		:		Obtenir Lettre de confirmation RIM
...
Historique des modifications :
						...
						2014-10-06					Pierre-Luc Simard			Ajout du paramètre @iID_Unite pour ne pas gérer de blob
																				Modifier plan pour régime dans le texte
...
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[psOPER_ObtenirLettreConfirmationRIM](
	@iConnectID INTEGER, -- ID de connexion de l'usager
	@iUnitIDs	INTEGER = NULL, -- ID du blob contenant les UnitID séparés par des « , » des groupes d’unités dont on veut générer le document.  		
	...
	@iID_Unite INTEGER) -- ID du groupe d'unité lorsque l'appel n'utilise pas le blob
AS
BEGIN
...
BEGIN TRY
	...
	IF @iUnitIDs IS NULL 
		INSERT INTO #UnitInReport (UnitID) 
			VALUES (@iID_Unite)
	ELSE
	...
	*/
	...
	INSERT INTO #Letter
		SELECT distinct
						...
						CASE WHEN SUB.LANGID = 'FRA'  THEN 
							CASE 
								WHEN (ISNULL(RND.RendInd, 0) <> 0) THEN 
									' et les revenus accumulés sur l’épargne au montant de ' + dbo.fn_Mo_MoneyToStr((RND.RendInd),SUB.LangID, 1)
								ELSE
									''
							END
						ELSE
							CASE 
								WHEN (ISNULL(RND.RendInd, 0) <> 0) THEN 
									', plus the income earned on contributions in the amount of ' + dbo.fn_Mo_MoneyToStr((RND.RendInd),SUB.LangID, 1) + ','
								ELSE
									''
							END
						END),
						...
						CASE WHEN SUB.LANGID = 'FRA'  THEN 
							CASE 
								...
								WHEN (ISNULL(COP.IQEE,0) + ISNULL(COP.IQEEMaj,0) <> 0) -- Avec IQEE et sans intérêt IQEE  
										AND ISNULL(COP.RendIQEE,0) + ISNULL(COP.RendIQEEMaj,0) + ISNULL(COP.RendIQEETin,0) = 0 
									THEN ' De même, nous avons transféré à votre nouveau plan l''Incitatif Québécois à l''Épargne-Études (IQEE) au montant de ' +  dbo.fn_Mo_MoneyToStr(ISNULL(COP.IQEE,0) + ISNULL(COP.IQEEMaj,0), SUB.LangID, 0) + ' $.'
								WHEN (ISNULL(COP.IQEE,0) + ISNULL(COP.IQEEMaj,0) <> 0) -- Avec IQEE et avec intérêt IQEE  
										AND ISNULL(COP.RendIQEE,0) + ISNULL(COP.RendIQEEMaj,0) + ISNULL(COP.RendIQEETin,0) <> 0 
									THEN ' De même, nous avons transféré à votre nouveau plan l''Incitatif Québécois à l''Épargne-Études (IQEE) au montant de ' +  dbo.fn_Mo_MoneyToStr(ISNULL(COP.IQEE,0) + ISNULL(COP.IQEEMaj,0), SUB.LangID, 0) + ' $ et l’intérêt de ' +  dbo.fn_Mo_MoneyToStr(ISNULL(COP.RendIQEE,0) + ISNULL(COP.RendIQEEMaj,0) + ISNULL(COP.RendIQEETin,0), SUB.LangID, 0) + ' $.'
							ELSE ''
							END
						ELSE
							CASE 
								...
								WHEN (ISNULL(COP.IQEE,0) + ISNULL(COP.IQEEMaj,0) <> 0) -- Avec IQEE et sans intérêt IQEE  
										AND ISNULL(COP.RendIQEE,0) + ISNULL(COP.RendIQEEMaj,0) + ISNULL(COP.RendIQEETin,0) = 0 
									THEN ' Also, we have transferred the amounts of $' + dbo.fn_Mo_MoneyToStr(ISNULL(COP.IQEE,0) + ISNULL(COP.IQEEMaj,0), SUB.LangID, 0)  + ' to your plan, this sum represents the Quebec Education Savings Incentive (QESI).'
								WHEN (ISNULL(COP.IQEE,0) + ISNULL(COP.IQEEMaj,0) <> 0) -- Avec IQEE et avec intérêt IQEE  
										AND ISNULL(COP.RendIQEE,0) + ISNULL(COP.RendIQEEMaj,0) + ISNULL(COP.RendIQEETin,0) <> 0 
									THEN ' Also, we have transferred the amounts of $' + dbo.fn_Mo_MoneyToStr(ISNULL(COP.IQEE,0) + ISNULL(COP.IQEEMaj,0), SUB.LangID, 0) + ' and $' + dbo.fn_Mo_MoneyToStr(ISNULL(COP.RendIQEE,0) + ISNULL(COP.RendIQEEMaj,0) + ISNULL(COP.RendIQEETin,0), SUB.LangID, 0) + ' to your plan, these sums represent the Quebec Education Savings Incentive (QESI) and its interest.'
							ELSE ''
							END
						END),
			OperBEC = (
			...
			Age = case when PL.iID_Regroupement_Regime = 2 then '18' else '19' end -- regroupement_regime 2 = reeeflex
		...
		JOIN Un_Oper O ON O.OperID = OpRIO.iID_OPER_RIO AND O.OperTypeID = 'RIM'
...
*/
		LEFT JOIN ( -- Recherche des intérêts sur individuelle générés après RIM - FT1
			SELECT
...
*/
	...
	SELECT 
		...
		BeneficiaryLastName, 
		RepresentantFirstName = 'zzz', --NULL,
		RepresentantLastName = 'zzz', --NULL,
		PlanDesc,
		Age
	FROM #Letter 
