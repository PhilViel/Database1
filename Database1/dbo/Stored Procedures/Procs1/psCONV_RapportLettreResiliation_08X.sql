...
Code du service		: psConv_RapportLettreResiliation_08X
Nom du service		: Générer la lettre de résiliaton 08X
But 				: 
Facette				: CONV
...
Exemple d’appel		:	EXEC psConv_RapportLettreResiliation_08X '2037998,U-20011219005,U-20011219006'
						EXEC psConv_RapportLettreResiliation_08X 'R-20050225003,R-20050225005'
...
****************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportLettreResiliation_08X] @conv varchar(max)
AS
BEGIN
	DECLARE
	...
	SELECT 
		...
		PlanClassification = [dbo].[fnCONV_ObtenirDossierClient](c.subscriberID,1) + '\' + replace(LEFT(CONVERT(VARCHAR, @today, 120), 10),'-','') + '_le_08x'
