...
Note                :	2004-06-28	Bruno Lapointe		Création
		ADX0000753	IA	2005-11-03	Bruno Lapointe		La procédure va chercher le montant du chèque
                        ...
                        2016-02-13  Maxime Martel       TI-6704 : ajout des champs langID, sexId, courriel, courriel valide, relevé papier

-- exec RP_UN_ScholarshipT4Formulatrix
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[RP_UN_ScholarshipT4Formulatrix] 
AS
BEGIN
	...
	SELECT 
        ...
        BeneficiarySex = BH.SexID,
        BeneficiaryLang = BH.LangID,
        Courriel = RTRIM(CoB.vcCourriel),
        CourrielValide = CASE WHEN CoB.bInvalide = 1 OR Cob.vcCourriel IS NULL THEN 'non'
                               ELSE 'oui'
                          END,
        RelevéPapier = CASE WHEN B.bReleve_Papier = 1 THEN 'oui'
                           ELSE 'non'
                       END,
		Country = A.CountryID,
    ...
    JOIN Un_Beneficiary B on B.BeneficiaryID = BH.HumanID
	LEFT JOIN dbo.Mo_Adr A ON A.AdrID = BH.AdrID
    LEFT JOIN dbo.fntGENE_CourrielEnDate_PourTous(NULL,NULL,1,1) CoB ON CoB.iID_Source = BH.HumanID
	WHERE 
		BH.ResidID = 'CAN'
