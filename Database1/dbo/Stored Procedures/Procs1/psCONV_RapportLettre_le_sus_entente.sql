...
Code du service		: psCONV_RapportLettre_le_sus_entente
Nom du service		: Générer la lettre d'arrêt temporaire des prélèvements automatiques
But 				: 
Facette				: CONV
...
Exemple d’appel		:	
						EXEC psCONV_RapportLettre_le_sus_entente 735155
						
drop procedure psCONV_RapportLettre_le_sus_entente
...
Historique des modifications:
		...
		------------	----------------------------------	-----------------------------------------	------------
		2014-07-11		Maxime Martel						Création du service	
...
****************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportLettre_le_sus_entente] @humanID integer
AS
BEGIN
	DECLARE
		@today datetime,
	...
	SELECT distinct
		...
		PlanClassification = [dbo].[fnCONV_ObtenirDossierClient](c.SubscriberID,1) + '\' + replace(LEFT(CONVERT(VARCHAR, @today, 120), 10),'-','')
							+ case HS.langID when 'FRA' then '_le_sus_entente' when 'ENU' then '_le_sus_entente_ang' end,
