...
Nom                 :	RP_UN_Scholarship1NoticeBatch
Description         :	Rapport de fusion Word des avis de premières bourses.
...
Note                :							2004-07-15	Bruno Lapointe		Création
						ADX0000706	IA	2005-07-13	Bruno Lapointe		Pas de lettre pour les bénéficiaires dont
												...
												2011-05-03	Pierre-Luc Simard	Retrait de la validation des 24 ans d'âge
...
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[RP_UN_Scholarship1NoticeBatch](
...
BEGIN
	...
	SELECT 
		@DocTypeID = DocTypeID
	FROM CRQ_DocType
	WHERE DocTypeCode = 'Scholarship1Notice'
	...
	INSERT INTO #Notice
		SELECT
			...
			SubscriberShortSexName = SHS.LongSexName,
		...
		WHERE S.ScholarshipNo = 1
			AND S.ScholarshipStatusID IN ('RES','ADM','WAI','TPA')
			--AND (YEAR(BH.BirthDate) > (SELECT ScholarshipYear FROM Un_Def) - 24)
...
-- EXEC RP_UN_Scholarship1NoticeBatch 1, 0