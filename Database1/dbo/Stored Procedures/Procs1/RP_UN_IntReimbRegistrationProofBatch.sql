...
Nom                 : RP_UN_IntReimbRegistrationProofBatch
...
Valeurs de retours  : @Dataset :
				...
				SubsFirstName		VARCHAR(35)		Prénom du souscripteur.
				SubsLastName		VARCHAR(50)		Nom du souscripteur.
				SubsAddress			VARCHAR(75)		Adresse du souscripteur.
				SubsCity			VARCHAR(100)	Ville du souscripteur.
				SubsState			VARCHAR(75)		Province du souscripteur.
				SubsZipCode			VARCHAR(10)		Code postal du souscripteur.
				SubsPhone			VARCHAR(27)		Téléphone du souscripteur.
				OfficePhone			VARCHAR(27)		Téléphone du souscripteur au travail.
				...
				BenefFirstName		VARCHAR(35)		Prénom du bénéficiaire.
				BenefLastName		VARCHAR(50)		Nom du bénéficiaire.
				GrantAmount			MONEY			Subvention.
				EstimatedIntReimbDate	DATETIME		Date estimée de remboursement intégral.
				FallingDueDate		DATETIME		Date butoir du souscripteur pour répondre à la lettre.
				PostponedIntReimbDate	DATETIME		Date estimée de remboursement intégral + 6 mois.
...
Note                :						2004-05-26	Bruno Lapointe		Création
					ADX0000626	IA	2004-12-17	Bruno Lapointe		Exclure les conventions résiliées
					ADX0000694	IA	2005-06-08	Bruno Lapointe		Replacer le paramètre @Date par @UnitIDs.
					ADX0001609	BR	2005-10-13	Bruno Lapointe		Gérer les cas qui n'ont pas de subventions.
...
exec RP_UN_IntReimbRegistrationProofBatch 816478,'133540',0
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[RP_UN_IntReimbRegistrationProofBatch](
...
BEGIN
	...
	INSERT INTO #UnitInReport
		...
		WHERE V.Val NOT IN (	SELECT V.Val
								FROM dbo.FN_CRQ_BlobToIntegerTable(@UnitIDs) V
								JOIN dbo.Un_Unit U ON U.UnitID = V.Val	
								JOIN CRQ_DocLink L ON L.DocLinkID = U.ConventionID AND L.DocLinkType = 1 
								JOIN CRQ_Doc D ON L.DocID = D.DocID -- Convention
								JOIN CRQ_DocTemplate DT ON DT.DocTemplateID = D.DocTemplateID AND DT.DocTypeID = @DocTypeID 
								)-- Enlever ceux dont la preuve dinscription a déjà été commandé	
	...
	INSERT INTO Un_IntReimbStep (
		...
		JOIN #RegistrationProof RP ON RP.ConventionNo = C.ConventionNo -- RP.UnitID = UIR.UnitID
