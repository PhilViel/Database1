...
Nom                 :	RP_UN_ScholarshipReminderNoticeBatch
Description         :	Rapport de fusion word des lettres de rappels aux boursiers par batch
Valeurs de retours  :	Dataset de données
Note                :					IA	2004-06-01	Bruno Lapointe		Création
								ADX0000704	IA 2005-07-05	Bruno Lapointe		Inscrit un historique des étapes (étape 3) de PAE.
															Aussi, on a modifier la lettre pour que dans le troisième champ
															d’information, qui contenait le numéro de bourse, il y est aussi le nom
															et prénom du souscripteur.  Donc on avait « 1 » et	on a maintenant
															« 1 – Ranger, Éric ».  Cela permet de trier les documents en ordre de
															numéro de bourse, nom et prénom du souscripteur pour l’impression dans la
...
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[RP_UN_ScholarshipReminderNoticeBatch] (
	@ConnectID INTEGER, -- ID de connexion de l'usager
	@ScholarshipIDs INTEGER, -- ID du blob contenant les ScholarshipID séparés par des , des bourses dont on veut générer le document.
...
BEGIN
	...
	INSERT INTO #Notice
		SELECT
			...
			ScholarshipNo =
				CASE BH.LangID
					WHEN 'FRA' THEN 
						CASE
							WHEN S.ScholarshipNo = 1 THEN '1iere'
						ELSE CAST(S.ScholarshipNo AS VARCHAR(2))+'e'
						END
					WHEN 'ENU' THEN 
						CASE
							WHEN S.ScholarshipNo IN (1,21,31) THEN CAST(S.ScholarshipNo AS varchar(2)) + 'st' 
							WHEN S.ScholarshipNo IN (2, 22) THEN CAST(S.ScholarshipNo AS varchar(2)) + 'nd'
							WHEN S.ScholarshipNo IN (3,23) THEN CAST(S.ScholarshipNo AS varchar(2)) + 'rd'  
						ELSE CAST(S.ScholarshipNo AS VARCHAR(2)) + 'th'
						END
		   	END,      
			LastScholarship =
				CASE SC.LastScholarshipID
					WHEN S.ScholarshipID THEN 1
				ELSE 0
				END, 
		...
		JOIN dbo.FN_CRQ_BlobToIntegerTable(@ScholarshipIDs) BS ON BS.Val = S.ScholarshipID
		...
		WHERE S.ScholarshipStatusID = 'WAI' -- Crée des lettres de bourses uniquement pour celles qui sont en attente
	...
	-- Inscrit un historique des étapes (étape 3) de PAE
	EXECUTE IU_UN_ScholarshipStep @ConnectID, @ScholarshipIDs, 3
