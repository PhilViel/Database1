...
Code de service		:		psOPER_ObtenirConventionIndividuelleRIO
Nom du service		:		Obtenir Convention Individuelle RIO
...
Historique des modifications :
						...
						----------					-------------------------------------	----------------------------		---------------
						2008-06-26					Jean-Francois Arial				Création de procédure stockée
						...
						2011-04-04					Frédérick Thibault				Modifications pour Prospectus 2010-2011
****************************************************************************************************/
CREATE PROCEDURE [dbo].[psOPER_ObtenirConventionIndividuelleRIO] (
...
AS
	BEGIN
		BEGIN TRY
			...
			FROM CRQ_DocType
			WHERE DocTypeCode = 'RIOConvIndividuel'
			...
			INSERT INTO #Convention
				SELECT
					...
					InitDepositAmount = dbo.fn_Mo_MoneyToStr(Co.Cotisation + Co.Fee, HS.LangID, 0),
					MemberFees = dbo.fn_Mo_MoneyToStr(ISNULL(SUM(FRS.mMontant_Frais), 0) + ISNULL(SUM(FRS.mMontant_TaxeTPS) , 0) + ISNULL(SUM(FRS.mMontant_TaxeTVQ) , 0), HS.LangID, 0),
					--TotalAmount = dbo.fn_Mo_MoneyToStr(CO2.Cotisation - ISNULL(SUM(FRS.mMontant_Frais), 0) - ISNULL(SUM(FRS.mMontant_TaxeTPS) , 0) - ISNULL(SUM(FRS.mMontant_TaxeTVQ) , 0), HS.LangID, 0)
				...
				JOIN (SELECT SUM(Cotisation) AS Cotisation, MAX(Fee) AS Fee, UnitID, RIO.iID_Convention_Destination FROM Un_Cotisation C1
						JOIN tblOper_OperationsRIO RIO ON (C1.OperId = RIO.iID_Oper_RIO) AND RIO.bRIO_Annulee = 0 AND RIO.bRIO_QuiAnnule = 0
						GROUP BY UnitId,  iID_Convention_Destination) AS CO 
						ON CO.UnitID = U.UnitID and CO.iID_Convention_Destination = OpRIO.iID_Convention_Destination
				...
				--			) FRS ON FRS.iID_Oper = (	SELECT iID_Operation_Enfant 
				--										FROM tblOPER_AssociationOperations 
				--										WHERE iID_Operation_Parent = opRIO.iID_Oper_RIO
				--										)
