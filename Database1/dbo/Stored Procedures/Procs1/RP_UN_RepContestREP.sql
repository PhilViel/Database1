...
Nom                 :	RP_UN_RepContestREP
Description         :	Procédure stockée du rapport : Concours
...
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[RP_UN_RepContestREP] (
...
BEGIN
	...
	INTO #MultFactor
	FROM dbo.Un_Unit U
	LEFT JOIN #NewSales N ON N.RepID = U.RepID AND U.UnitID = N.UnitID
	LEFT JOIN #Terminated T ON T.RepID = U.RepID AND U.UnitID = T.UnitID
	JOIN Un_RepContestUnitMultFactorCfg C ON C.RepContestCfgID = @RepContestCfgID
														AND (C.StartDate <= U.dtFirstDeposit)
														AND (C.EndDate IS NULL OR C.EndDate >= U.dtFirstDeposit)
	...
	IF DATEDIFF(SECOND, @dtBegin, @dtEnd) > @siTraceReport
		...
		INSERT INTO Un_Trace (
			...
			SELECT
				...
				'Concours',
				'RP_UN_RepContestREP',
				'EXECUTE RP_UN_RepContestREP @ConnectID = '+CAST(@ConnectID AS VARCHAR)+
					', @RepContestCfgID = '+CAST(@RepContestCfgID AS VARCHAR)
END

/*  Sequence de test - par: PLS - 09-05-2008
	exec [dbo].[RP_UN_RepContestREP] 
