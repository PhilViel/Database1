...
exec GU_RP_DetailUniteConcours2010 '2009-10-07', '2010-10-01', NULL, NULL, 149497, 'R'
*********************************************************************************************************************/
CREATE procedure [dbo].[GU_RP_DetailUniteConcours2010] 
	(
...
if @StartDateMaj is NOT null and @EndDateMaj is Null
	...
	EXEC SL_UN_RepGrossANDNetUnits --NULL, @StartDate, @EndDate, 0 , 1 -- on va chercher toutes les données et on filtre par repid à la fin
	...
	select 
		...
		--BrutRFlex = (case when u.dtfirstdeposit between @StartDateMaj and  @EndDateMaj then 1.35 else 1 end) * (case when c.planid  in (10,12) then Brut else 0 end),
		BrutRFlex = case when c.planid  in (10,12) then Brut else 0 end,
		--RetraitsRFlex = (case when u.dtfirstdeposit between @StartDateMaj and  @EndDateMaj then 1.35 else 1 end) * (case when c.planid  in (10,12) then Retraits else 0 end),
		RetraitsRFlex = case when c.planid  in (10,12) then Retraits else 0 end,
		--ReinscriptionsRFlex = (case when u.dtfirstdeposit between @StartDateMaj and  @EndDateMaj then 1.35 else 1 end) * (case when c.planid  in (10,12) then Reinscriptions else 0 end),
		ReinscriptionsRFlex = case when c.planid  in (10,12) then Reinscriptions else 0 end,
		--EstMajore = case when (u.dtfirstdeposit between @StartDateMaj and  @EndDateMaj and c.planid  in (10,12)) then 'Oui' else '' end ,
		EstMajore = '',
		u.dtfirstdeposit,
		u.SignatureDate,
		u.TerminatedDate,
		DateCodage = SD.StartDate,
		--NetMajore = (Brut - Retraits + Reinscriptions) * (case when (c.planid in (10,12) and u.dtfirstdeposit between @StartDateMaj and  @EndDateMaj) then 1.35 else 1 end) 
