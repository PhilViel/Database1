...
Nom                 :	psREPR_RapportMortier_2019
Description         :	pour le rapport SSRS "Mortier 2018"
Valeurs de retours  :	Dataset 
Note                :	2018-06-29	Donald Huppé	jira prod-10459 : Créaton à partir de psREPR_RapportMortier_2018
*********************************************************************************************************************/

--  exec psREPR_RapportMortier_2018 '2018-01-01', '2018-06-29' , '2017-01-01', '2017-06-29'

CREATE PROCEDURE [dbo].[psREPR_RapportMortier_2019] 
...
create table #VenteDirPrec (
	...
	where rep.BusinessEnd is null
		and g.RepID not in (
				-- DIRECTEURS ADJOINTS
					149497, --	Carole Marchand
					422223, --	Line Durivage
					466100, --	Chantal Jobin
					500292, --	Myriam Derome
					629154, --	Steve Blais
					655109, --	Anne LeBlanc-Levesque
					676177, --	Amélie Rancourt-Fortin
					702402 --	Véronic Bénard
				)
	group by
		case 
		when G.repid in (580886,584960) then 580886 -- Jean-François Gemme
	...
	WHERE ISNULL(r.BusinessEnd,'9999-12-31') >= '2018-01-02' -- nouveau 2019 : seulement les rep qui était actif au début de l'année
	group by
		G.BossID,
		Rep.RepCode,
		HRep.firstName + ' ' + HRep.LastName,
	...
	from
		...
		where G.repID <> 496768 -- exclure l'ancien directeur Vahan Matossian
			AND ISNULL(r.BusinessEnd,'9999-12-31') >= '2018-01-02' -- nouveau 2019 : seulement les rep qui était actif au début de l'année
	group by
		G.BossID,
		Rep.RepCode,
		HRep.firstName + ' ' + HRep.LastName,
