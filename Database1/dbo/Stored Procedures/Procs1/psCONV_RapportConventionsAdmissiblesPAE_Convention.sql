...
Exemple d’appel		:	
	...
	EXEC psCONV_RapportConventionsAdmissiblesPAE 
	EXEC psCONV_RapportConventionsAdmissiblesPAE_Convention 'C-20010323021' -- Pour une convention
...
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportConventionsAdmissiblesPAE_Convention] 
(
...
BEGIN
    ...
    SELECT 
        ...
        CD.QuotePart  
    INTO #tConvColAdmPAE
    FROM dbo.fntCONV_ObtenirValeursPAECollectifDisponible(@iID_Convention) CD
    ...
    JOIN fntCONV_ObtenirStatutConventionEnDate_PourTous(NULL, @iID_Convention) CS ON CS.ConventionID = C.ConventionID
	...
	WHERE CE.ConventionID = ISNULL(@iID_Convention, CE.ConventionID)
        AND O.OperDate <= GETDATE()
    ...
    LEFT JOIN (
        ...
        WHERE S.ConventionID = ISNULL(@iID_Convention, S.ConventionID)
            AND S.ScholarshipStatusID = 'PAD' --IN ('24Y','25Y','DEA','PAD','REN')
