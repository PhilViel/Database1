...
Code du service		: psCONV_RapportLettre_le_resil_aucun_depot (remplace psConv_RapportLettreResiliation_08X)
Nom du service		: Générer la lettre de résiliaton aucub dépôt
But 				: 
Facette				: CONV
...
Exemple d’appel		:	EXEC psCONV_RapportLettre_le_resil_aucun_depot '2037998,U-20011219005,U-20011219006'
						EXEC psCONV_RapportLettre_le_resil_aucun_depot 'R-20050225003,R-20050225005'
...
****************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportLettre_le_resil_aucun_depot] @conv varchar(max)
AS
BEGIN
	DECLARE
	...
	SELECT 
		...
		PlanClassification = [dbo].[fnCONV_ObtenirDossierClient](c.subscriberID,1) + '\' + replace(LEFT(CONVERT(VARCHAR, @today, 120), 10),'-','') + '_le_resil_0'
		,SexRep = hr.SexID
