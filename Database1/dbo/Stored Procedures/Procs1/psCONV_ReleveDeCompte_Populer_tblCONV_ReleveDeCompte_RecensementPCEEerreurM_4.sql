...
Nom                 :	psCONV_ReleveDeCompte_Populer_tblCONV_ReleveDeCompte_RecensementPCEEerreurM_4
Description         :	
...
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_ReleveDeCompte_Populer_tblCONV_ReleveDeCompte_RecensementPCEEerreurM_4] (
	@NoErreur varchar(2)
)
AS
BEGIN
...
if @NoErreur = '4'
	begin
	if exists (select * from sysobjects where name = 'tblCONV_ReleveDeCompte_RecensementPCEEerreur4')
		delete from tblCONV_ReleveDeCompte_RecensementPCEEerreur4 --drop table ReleveDeCompte_RecensementPCEEerreur4 -- select * from tblCONV_ReleveDeCompte_RecensementPCEEerreur4
	end


if @NoErreur = 'M'
	begin
	if exists (select * from sysobjects where name = 'tblCONV_ReleveDeCompte_RecensementPCEEerreurM')
		delete from tblCONV_ReleveDeCompte_RecensementPCEEerreurM --drop table ReleveDeCompte_RecensementPCEEerreurM -- select * from tblCONV_ReleveDeCompte_RecensementPCEEerreurM
	end
...
--SELECT * from #tListeBenefL1_1erTransdurandPeriode
--return

	-- Liste des conventions avec au moins une erreur @NoErreur en cours
	SELECT 
		C4.ConventionID
		--,Co.BeneficiaryID
		,C4.iCESP400ID
	...
	WHERE C4.iCESPSendFileID IS NOT NULL -- Être envoyé au PCEE
		...
		--AND C4.dtTransaction BETWEEN @StartDate AND @EndDate -->= @StartDate
		AND C4.dtTransaction <= @EndDate -->= @StartDate
...
--select * from #ACESG where ACESG <> 0 order by iCESP400ID
	...
	--select * from #tblbenef
	create index #indbenef on #tblbenef(BeneficiaryID)
	...
	INTO #tListeBenefL -- drop table #tListeBenefL -- select * from #tListeBenefL
	FROM UN_CESP400 C4
	...
	left JOIN #tblbenef -- benef avec dernière raison de réponse reçu = @NoErreur
	...
	WHERE 
			...
			AND Co.bSendToCESP  = 1 -- Doit être envoyée au PCEE
			AND S.iCESPReceiveFileID IS NOT NULL -- Avoir reçu une réponse
			AND C9.cCESP900ACESGReasonID IN ('1', '2', '3', '4', '5', '6', '7', '9', 'B', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'L', 'N') -- Le NAS ne correspond pas avec celui de l'ARC
			AND R4.iCESP400ID IS NULL -- Pas annulé
		...
		--and Co.ConventionNo = 'R-20031014003'
		--and Co.BeneficiaryID = 241388
	...
	--SELECT '#tListeBenefL',* from #tListeBenefL
...
/****************************************************************************************************/
	...
	WHERE C4.iCESPSendFileID IS NOT NULL -- Être envoyé au PCEE
		...
		--AND C4.dtTransaction >= @StartDate
...
--SELECT  t2= '2',* from #tListeBenef  

--return

--SELECT * FROM Un_Convention WHERE ConventionNo = 'R-20060831044'
...
--SELECT  t6= '6',* from #tListeBenef  --where BeneficiaryID = 511745
...
--SELECT  t7= '7',* from #tListeBenef --where BeneficiaryID = 511745
...
if @NoErreur = '4'
	...
	-- Final
	insert into tblCONV_ReleveDeCompte_RecensementPCEEerreur4
	SELECT DISTINCT
		C.BeneficiaryID,
	...
	--into tblCONV_ReleveDeCompte_RecensementPCEEerreur4
	FROM #tListeBenef LB
		/*
		(
...
if @NoErreur = 'M'
	...
	-- Final
	insert into tblCONV_ReleveDeCompte_RecensementPCEEerreurM 
	SELECT DISTINCT
		C.BeneficiaryID,
		NomBeneficiaire = HB.LastName,
		PrenomBeneficiaire = HB.FirstName,
		...
		NomRep = HR.LastName,
		PrenomRep = HR.FirstName 
	--into tblCONV_ReleveDeCompte_RecensementPCEEerreurM
	FROM #tListeBenef LB
	/*(
		-- Va chercher la convention créée en dernier pour un bénéficiaire, 
		-- selon le lien de parenté le plus proche trouvé
