...
Code du service		: psCONV_RapportLettre_le_resil_60jrs (remplace psConv_RapportLettreResiliation_08_60)
...
Exemple d’appel		:	EXEC psCONV_RapportLettre_le_resil_60jrs '2037998,U-20011219005,U-20011219006'
						EXEC psCONV_RapportLettre_le_resil_60jrs 'R-20050225003,R-20050225005'
...
****************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportLettre_le_resil_60jrs] @conv varchar(max)
AS
BEGIN
	DECLARE
	...
	SELECT 
		...
		sum(Co.Fee) + sum(Co.Cotisation) AS montantFrais,
		...
		PlanClassification = [dbo].[fnCONV_ObtenirDossierClient](c.subscriberID,1) + '\' + replace(LEFT(CONVERT(VARCHAR, @today, 120), 10),'-','') + '_le_resil_60jrs'
		,SexRep = hr.SexID
