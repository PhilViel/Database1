...
Code du service		: psCONV_RapportLettre_le_2_NSF (comme le_arret_paiement)
Nom du service		: Générer la lettre d'arrêt de paiement "le_2_NSF"
But 				: 
Facette				: CONV
...
EXEC psCONV_RapportLettre_le_2_NSF 809999, 242.21
...
Historique des modifications:
		...
		------------	----------------------------------	-----------------------------------------	------------
		2018-08-27		Donald Huppé						Création du service	
		
****************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportLettre_le_2_NSF] 
	@SubscriberID INT,
...
BEGIN
	...
	SELECT distinct
		...
		PlanClassification = [dbo].[fnCONV_ObtenirDossierClient](c.SubscriberID,1) + '\' + replace(LEFT(CONVERT(VARCHAR, @today, 120), 10),'-','')
							+ case HS.langID when 'FRA' then '_le_2_NSF' when 'ENU' then '_le_2_NSF_ang' end
