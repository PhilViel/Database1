...
Code du service		: psCONV_RapportLettre_le_NSF_Avance
Nom du service		: Générer la lettre d'arrêt de paiement "le_NSF_Avance"
But 				: 
Facette				: CONV
...
Paramètres d’entrée	:	Paramètre					Description
						--------------------------	-----------------------------------------------------------------
						@SubscriberID
						@@DateProchPrelevement
						@ProchainMontantAverser

Exemple d’appel		:	

EXEC psCONV_RapportLettre_le_NSF_Avance 809999, '2018-11-19', 242.21
...
Historique des modifications:
		...
		------------	----------------------------------	-----------------------------------------	------------
		2018-10-29		Donald Huppé						Création du service	
		
****************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportLettre_le_NSF_Avance] 
	@SubscriberID INT,
	@DateProchPrelevement DATETIME,
	@ProchainMontantAverser MONEY
AS
BEGIN
	DECLARE
		@today datetime,
	...
	SELECT 
		...
		MontantPrelevement = SUM(V.MontantPrelevement),
	...
	SELECT distinct
		...
		P.NbPrelevement,
		P.NbConv,
		DateProchPrelevement = @DateProchPrelevement,
		...
		PlanClassification = [dbo].[fnCONV_ObtenirDossierClient](c.SubscriberID,1) + '\' + replace(LEFT(CONVERT(VARCHAR, @today, 120), 10),'-','')
							+ case HS.langID when 'FRA' then '_le_NSF_Avance' when 'ENU' then '_le_NSF_Avance_ang' end
