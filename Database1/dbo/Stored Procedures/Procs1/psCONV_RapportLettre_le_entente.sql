...
Code du service		: psCONV_RapportLettre_le_entente
...
Exemple d’appel		:	
						EXEC psCONV_RapportLettre_le_entente 735155
						
drop procedure psCONV_RapportLettre_le_entente
...
Historique des modifications:
		...
		------------	----------------------------------	-----------------------------------------	------------
		2014-08-22		Donald Huppé						Création du service							pareil comme psCONV_RapportLettre_le_SUS_entente
...
****************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportLettre_le_entente] @humanID integer
AS
BEGIN
	DECLARE
		@today datetime,
	...
	SELECT distinct
		...
		PlanClassification = [dbo].[fnCONV_ObtenirDossierClient](c.SubscriberID,1) + '\' + replace(LEFT(CONVERT(VARCHAR, @today, 120), 10),'-','')
							+ case HS.langID when 'FRA' then '_le_entente' when 'ENU' then '_le_entente_ang' end,
