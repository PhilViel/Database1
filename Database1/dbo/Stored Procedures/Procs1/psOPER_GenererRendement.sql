/****************************************************************************************************
Code de service :	psOPER_GenererRendement
Nom du service	 :	TBLOPER_RENDEMENTS (Rechercher les taux de rendement)	
But			 :	Générer le calcul des rendements	
Description	 :	Pour une date de calcul, le service permet de générer les rendements, selon les types de rendement saisis
Facette		 :	OPER
...
Parametres d'entrée :   Parametres		    Description
				    ...
				    @bActiveDebug	    Flag qui permet d'activer le débogage qui retourne le contenu de la table temporaire 
...
Exemple d'appel:
	   DECLARE @i INT
	   EXECUTE @i = dbo.psOPER_GenererRendement 114404, 'S', 1
	   PRINT 'i = ' + CAST(@i AS VARCHAR(6))

	   DECLARE @i INT
	   EXECUTE @i = dbo.psOPER_GenererRendement NULL, 'C', 0
	   PRINT 'i = ' + CAST(@i AS VARCHAR(6))
...
Historique des modifications :
	    ...
	    2009-08-25      Jean-François Arial			Remplacer la date de prospectus par un paramètre applicatif
	    2009-08-25      Jean-François Arial			Ajustement des conditions des dates pour les RIO
	    2009-08-27      Jean-François Arial			Ajout des conditions sur le code de rendement pour les 8 types
	    2009-08-28      Jean-François Arial			Changement des conditions sur la date de prospectus et ajustement sur la création d'une opération
	    ...
	    2009-09-09      Jean-François Gauthier		Ajustement de la variable @dTaux_Rendement à décimal(10,3)
											        ...
											        Modification au point 1.3.2.3.5 et 1.3.2.3.6 : les transaction IQI passe de l'un à l'autre
	    ...
	    2009-09-22      Jean-François Gauthier		Ajout de la validation du iID_OPER dans le WHERE des DELETE sur Un_ConventionOPER car dans la même génération, 
                                                    on pouvait effacer des transactions générées dans un taux précédents si le type d'opération est le même (cas 1.3.2.3.7 et 1.3.2.3.8)
											        Modification du WHERE des DELETE pour les taux .7 et .8 afin de passer par la table tblOPER_TauxRendement pour 
                                                    supprimer dans Un_ConventionOPER
	    ...
	    2010-04-19      Éric Deshaies				Faire le retransfert RIO uniquement s'il y a un solde positif.  Analyse de retransfert des soldes négatifs à faire plus tard.
	    2010-04-21      Jean-François Gauthier		Modification afin d'effectuer le retransfert RIO uniquement dans les cas de taux modifiés ou de l'ajout de nouveaux taux
											        Modification à l'appel de la fonction fntOPER_RechercherRendement pour lui ajouter le paramètre @bAfficher
	    ...
	    2011-04-28      Frédérick Thibault			Adaptation de l'appel à psOPER_CreerOperationRIO pour projet Prospectus 2010-2011 (FT1)
	    2011-06-03      Frédérick Thibault			Ajout de la validation (fntOPER_ValiderRetransfertRIO) pour le retransfert (FT2)
	    2012-04-18      Frédérick Thibault			Exclusion des 'TRI' de la table #tRIO
	    2013-01-09      Frédérick Thibault			Ajout du traitement par Fiducie et par délai de date RI estimée (FT3)
	    2013-03-21      Frédérick Thibault			Abolition du rendement sur les frais (fee) à la convention individuelle issue d'un TRI (FT4)
	    2013-06-04      Frédérick Thibault			Abolition du rendement sur les conventions qui ont fait l'objet d'un RIO post-daté ou non (FT5)
	    2013-11-22      Pierre-Luc Simard			Exclusion des TRI dans la table #tRIO
	    2015-01-06	    Donald Huppé				Modifications pour améliorer la vitesse (INSERT INTO	dbo.Un_ConventionOper)
        2015-12-01      Steeve Picard               Utilisation de la nouvelle fonction «fntCONV_ObtenirStatutConventionEnDate_PourTous»
        2018-05-02      Pierre-Luc Simard           Calculer des rendements même après un RIN partiel sur ces deux types:
                                                        5. Épargne et frais après la date estimée du RI (Rend. RI)
                                                        6. Revenus acc. sur l'épargne et frais après la date estimée du RI (Rend. RI)
N.B.
	Consulter TT_UN_MonthlyConventionInterest pour avoir une idée du traitement

-- EXECUTE dbo.psOPER_GenererRendement NULL, 'S', 0
 ****************************************************************************************************/
CREATE PROCEDURE [dbo].[psOPER_GenererRendement]
(
	@iID_Convention    INT,
	@cEtat		    CHAR(1),
	@bActiveDebug	    BIT	= 0
...
BEGIN
    ...
    create table #tblTransactionConvention	--TABLE
										  (
										  iID_Convention			INT 
										  ,iJourOperation			INT
										  ,mMontantRI				MONEY
...
/*	MIS EN COMMENTAIRE LE 2009-11-03
	   ...
	   SELECT
		  ConventionID, StartDate
	   FROM
        	  dbo.fntCONV_ObtenirStatutConventionEnDate_PourTous(GetDate(), NULL)
	   WHERE
		  ConventionID = ISNULL(@iID_Convention, ccvt.ConventionID)
*/
	   ...
	   WHERE 
		  ...
		  AND 
		  rio.OperTypeID IN ('RIO', 'RIM')
					
	
	   -- Table pour exclure les conventions (source) RIO des rendements (FT5)
	   create table	#tConvSourceRIO (iID_Convention_Source INT)

	   INSERT INTO #tConvSourceRIO (iID_Convention_Source)
	   SELECT DISTINCT rio.iID_Convention_Source
	   FROM dbo.tblOPER_OperationsRIO rio
	   WHERE rio.bRIO_Annulee = 0 
	   ...
	   WHILE @@FETCH_STATUS = 0
		  BEGIN
				...
				SELECT conventionID, StartDate
                    FROM dbo.fntCONV_ObtenirStatutConventionEnDate_PourTous(GetDate(), NULL)
				...
				WHERE 
				    NOT EXISTS
								    (
									   ...
									   FROM 
										  dbo.Un_Unit u
                                          LEFT JOIN dbo.fntCONV_ObtenirStatutRINUnite(NULL, NULL, @dtDernierJourDuMoisATraiter) RIN ON RIN.UnitID = u.UnitID
										  INNER JOIN dbo.Un_Modal m
												ON m.ModalID = u.ModalID
				    ...
				    SELECT						-- Transactions ANTÉRIEURES au mois à traiter (soldes d'épargnes et frais)
					   c.ConventionID
					   ,case when o.OperDate BETWEEN @dtPremierJourDuMoisATraiter and @dtDernierJourDuMoisATraiter then Day(o.OperDate) when o.OperDate < @dtPremierJourDuMoisATraiter then 0 end
					   ,CASE	WHEN ISNULL(RIO.OperTypeID, '') <> 'TRI' -- FT4
								THEN 
				    ...
				    FROM
					   #tblUniteEligible ue
					   INNER JOIN	dbo.Un_Unit u ON u.UnitID = ue.iID_Unit
					   INNER JOIN	dbo.Un_Convention c ON u.ConventionID = c.ConventionID 
                            INNER JOIN	dbo.fntCONV_ObtenirStatutConventionEnDate_PourTous(GetDate(), NULL) cs ON cs.conventionID = c.ConventionID
					   INNER JOIN	dbo.Un_Cotisation ct ON ct.UnitId = u.UnitId
					   INNER JOIN	dbo.Un_Oper o ON ct.OperID = o.OperID	
					   --INNER JOIN	#tConventionState tmp ON ccs.ConventionID = tmp.ConventionID AND ccs.StartDate = tmp.MostRecentDate
					   LEFT JOIN tblOPER_OperationsRIO RIO ON RIO.iID_Oper_RIO = o.OperID -- FT4
					   LEFT join  #tConvSourceRIO rio1 on rio1.iID_Convention_Source = c.ConventionID --and rio.iID_Convention_Source is null
				    WHERE	c.ConventionID = ISNULL(@iID_Convention, c.ConventionID)
					   AND	cs.ConventionStateID IN ('REE','TRA')	-- Type RÉÉÉ ou Transitoire
					   AND	o.OperDate < @dtDernierJourDuMoisATraiter
					   AND ISNULL(RIO.bRIO_Annulee, 0) = 0
					   AND ISNULL(RIO.bRIO_QuiAnnule, 0) = 0
					   and rio1.iID_Convention_Source is null--AND c.ConventionID NOT IN (SELECT iID_Convention_Source FROM #tConvSourceRIO WHERE iID_Convention_Source = c.ConventionID) -- FT5
				    ...
				    FROM
					   ...
					   INNER JOIN	dbo.fntCONV_ObtenirStatutConventionEnDate_PourTous(GetDate(), NULL) cs	ON c.ConventionID = cs.ConventionID		
					   INNER JOIN dbo.Un_Cotisation ct
						  ON ct.UnitId = u.UnitId
					   INNER JOIN dbo.Un_Oper o
						  ON ct.OperID = o.OperID	
					   --INNER JOIN #tConventionState tmp ON ccs.ConventionID = tmp.ConventionID AND	ccs.StartDate = tmp.MostRecentDate
					   -- INNER JOIN	dbo.Un_ConventionState cs	ON cs.ConventionStateID = ccs.ConventionStateID 
					   LEFT JOIN tblOPER_OperationsRIO RIO ON RIO.iID_Oper_RIO = o.OperID -- FT4
					   LEFT join  #tConvSourceRIO rio1 on rio1.iID_Convention_Source = c.ConventionID --and rio.iID_Convention_Source is null
				    WHERE
					   c.ConventionID = ISNULL(@iID_Convention, c.ConventionID)
				    ...
				    select OperID, iID_Convention,vcConventionOperTypeMntSouscrit,Rend
				    from (

				    SELECT 
					   OperID = @iID_Oper
					   ,iID_Convention = tc.iID_Convention						
					   ,vcConventionOperTypeMntSouscrit = @vcConventionOperTypeMntSouscrit
					   ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0)) -- FT3
				    FROM
					   #tblTransactionConvention tc						
				    GROUP BY
					   tc.iID_Convention
				    --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0)) <> 0
				    ...
				    SELECT 
					   ...
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0)) -- FT3
				    FROM
					   #tblTransactionConvention tc						
				    GROUP BY
					   tc.iID_Convention
				    HAVING 
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0)) <> 0
				    */
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0))
...
-- RENDEMENT SUR ÉPARGNE ET FRAIS DE CONVENTIONS INDIVIDUELLES ISSUES D'UN RIO (NOUVELLES ET EXISTANTES)
				    ...
				    FROM 
					   ...
					   INNER JOIN	dbo.fntCONV_ObtenirStatutConventionEnDate_PourTous(GetDate(), NULL) cs	ON co.ConventionID = cs.ConventionID
					   INNER JOIN dbo.Un_Modal m 
						  ON m.ModalID = u.ModalID
					   INNER JOIN dbo.Un_Plan p 
						  ON p.PlanID = m.PlanID
					   ...
					   INNER JOIN dbo.Un_Oper o 
						  ON o.OperID = ct.OperID 
					   --INNER JOIN #tConventionState tmp ON ccs.ConventionID = tmp.ConventionID AND	ccs.StartDate = tmp.MostRecentDate
					   INNER JOIN #tRIO rio ON rio.iID_Convention_Destination = u.ConventionID
				    WHERE 
					   ...
					   AND cs.ConventionStateID IN ('REE','TRA')	-- Type RÉÉÉ ou Transitoire
					   AND
					   o.OperDate < @dtDernierJourDuMoisATraiter -- modifié par JFA 2009-08-28	
					   --AND
					   --rio.OperTypeID IN ('RIO', 'RIM')
				    GROUP BY 
					   u.ConventionID
				    ...
				    */
				    ...
				    SELECT 
					   ...
					   ,tc.iID_Convention						
					   ,@vcConventionOperTypeMntSouscrit
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
		  ...
		  BEGIN
				...
				WHERE 
				    ...
				    FROM
					   ...
					   INNER JOIN dbo.Un_Oper o
						  ON ct.OperID = o.OperID	
					   INNER JOIN dbo.fntCONV_ObtenirStatutConventionEnDate_PourTous(GetDate(), NULL) cs
						    ON C.conventionid = cs.conventionid
				    WHERE
					   c.ConventionID = ISNULL(@iID_Convention, c.ConventionID)
					   AND cs.ConventionStateID in ('REE','TRA') -- je veux les convention qui ont cet état
					   AND o.OperDate < @dtDernierJourDuMoisATraiter
					   AND c.ConventionID NOT IN (SELECT iID_Convention_Source FROM #tConvSourceRIO WHERE iID_Convention_Source = c.ConventionID) -- FT5
				    GROUP BY
					   c.ConventionID
					   ,case when o.OperDate BETWEEN @dtPremierJourDuMoisATraiter and @dtDernierJourDuMoisATraiter then Day(o.OperDate) when o.OperDate < @dtPremierJourDuMoisATraiter then 0 end
				    HAVING
				    ...
				    FROM 
					   ...
					   INNER JOIN dbo.fntCONV_ObtenirStatutConventionEnDate_PourTous(GetDate(), NULL) cs
						    ON cop.conventionid = cs.conventionid
				    WHERE 
					   cop.ConventionId = ISNULL(@iID_Convention, cop.ConventionID)
				        AND cs.ConventionStateID in ('REE','TRA') -- je veux les convention qui ont cet état
					   AND cop.OperID IN (	SELECT 
...
-- RENDEMENT SUR ÉPARGNE ET FRAIS DE CONVENTIONS INDIVIDUELLES ISSUES D'UN RIO (NOUVELLES ET EXISTANTES)
				    ...
				    FROM 
					   ...
					   INNER JOIN #tRIO rio
						  ON rio.iID_Convention_Destination = u.ConventionID
					   INNER JOIN dbo.fntCONV_ObtenirStatutConventionEnDate_PourTous(GetDate(), NULL) cs
						    ON co.conventionid = cs.conventionid
				    WHERE 
					   co.ConventionId = ISNULL(@iID_Convention, co.ConventionID)
				        AND cs.ConventionStateID in ('REE','TRA') -- je veux les convention qui ont cet état
					   AND
					   p.PlanTypeID = 'IND'
					   AND
				...
				-- Print '4'
				    ...
				    from (
					   SELECT 
						  ...
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeMntSouscrit = @vcConventionOperTypeMntSouscrit
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI2,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0)) -- FT3
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI2,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0)) <> 0
					   ) V
				    where V.Rend <> 0
				    /*
				    SELECT 
					   ...
					   ,tc.iID_Convention						
					   ,@vcConventionOperTypeMntSouscrit
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI2,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
				    HAVING
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI2,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0)) <> 0
				    */
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI2,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0))
				    --															FROM	#tblTransactionConvention tc),0)
    ...
    -- RENDEMENT SUR REVENUS ACCUMULÉS DES CONVENTIONS NOUVELLES ET EXISTANTES
				    ...
				    from (
					   SELECT 
						  ...
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeMntSouscrit = @vcConventionOperTypeMntSouscrit
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantITR,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0)) -- FT3
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantITR,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0)) <> 0
					   ) V
				    where V.Rend <> 0
				    /*
				    SELECT 
					   ...
					   ,tc.iID_Convention						
					   ,@vcConventionOperTypeMntSouscrit
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantITR,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
				    HAVING
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantITR,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0)) <> 0
				    */

				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantITR,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 0))
				    --															FROM	#tblTransactionConvention tc),0)
    ...
    BEGIN
				...
				-- "fPG" à celui de la subvention fédérale de base "fCESG"
				    ...
				    FROM
					   ...
					   --INNER JOIN #tConventionState tmp								ON ccs.ConventionID = tmp.ConventionID AND	ccs.StartDate = tmp.MostRecentDate
					   LEFT join  #tConvSourceRIO rio on rio.iID_Convention_Source = c.ConventionID --and rio.iID_Convention_Source is null
				    WHERE
					   c.ConventionID = ISNULL(@iID_Convention, c.ConventionID)
					   AND				cs.ConventionStateID IN ('REE','TRA')	-- Type RÉÉÉ ou Transitoire
					   AND	o.OperDate < @dtDernierJourDuMoisATraiter
				    ...
				    -- Print '5.1 new ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    -- select '5.1 new ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
	   ...
	   /*				
				    ...
				    WHERE
					   ...
					   and c.ConventionID = ISNULL(@iID_Convention, c.ConventionID)
					   --AND c.ConventionID NOT IN (SELECT iID_Convention_Source FROM #tConvSourceRIO WHERE iID_Convention_Source = c.ConventionID) -- FT5
					   and rio.iID_Convention_Source is null
				    GROUP BY
					   c.ConventionID			
				    ...
				    -- Print '5.2 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    -- select '5.2 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    */
				    ...
				    select OperID, iID_Convention,vcConventionOperTypeSCEE,Rend
				    from (
					   SELECT 
						  OperID = @iID_Oper
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeSCEE = @vcConventionOperTypeSCEE
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCESG,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) -- FT3
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCESG,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
					   ) V
				    where V.Rend <> 0
				    /*
				    SELECT 
					   ...
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCESG,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
				    HAVING 
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCESG,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
				    */
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCESG,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
																			 
						
				    -- 2. Insertion des rendements générés pour les SCEE bonifié "IS+"

				    -- Print '5.3 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    -- select '5.3 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)

				    INSERT INTO	dbo.Un_ConventionOper
				    (
					   OperID, ConventionID, ConventionOperTypeID, ConventionOperAmount
				    ) 
				    select OperID, iID_Convention,vcConventionOperTypeSCEEPlus,Rend
				    from (
					   SELECT 
						  OperID = @iID_Oper
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeSCEEPlus = @vcConventionOperTypeSCEEPlus
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantACESG,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) -- FT3
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantACESG,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
					   ) V
				    where V.Rend <> 0
				    /*
				    SELECT 
					   ...
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantACESG,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
				    HAVING
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantACESG,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
				    */
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantACESG,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
																					
																						
				    -- 3. Insertion des rendements générés pour les types BEC "IBC"

				    -- Print '5.4 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    -- select '5.4 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)

				    INSERT INTO	dbo.Un_ConventionOper
				    (
					   OperID, ConventionID, ConventionOperTypeID, ConventionOperAmount
				    ) 
				    select OperID, iID_Convention,vcConventionOperTypeBEC,Rend
				    from (
					   SELECT 
						  OperID = @iID_Oper
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeBEC = @vcConventionOperTypeBEC
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCLB,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) -- FT3
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCLB,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
					   ) V
				    where V.Rend <> 0
				    /*
				    SELECT 
					   ...
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCLB,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
				    HAVING
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCLB,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
				    */
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCLB,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
				...
				--  ****** DÉBUT DU TRAITEMENT POUR LES TRANSACTIONS ANTÉRIEURES AU PREMIER JOUR DU MOIS À TRAITER ********				
				    ...
				    WHERE 
					   c.ConventionId = ISNULL(@iID_Convention, c.ConventionID)
				        AND cs.ConventionStateID in ('REE','TRA') -- je veux les convention qui ont cet état
						  AND
						  c.OperID IN (	SELECT ISNULL(tx.iID_Operation,-1) 
										  FROM 
												dbo.tblOper_TauxRendement tx
												INNER JOIN dbo.tblOPER_Rendements r
												    ON tx.iID_Rendement = r.iID_Rendement
    ...
    BEGIN
				...
				-- Print '6'
				    ...
				    WHERE
					   ...
					   AND		cs.ConventionStateID IN ('REE','TRA')	-- Type RÉÉÉ ou Transitoire
					   AND
					   o.OperDate < @dtDernierJourDuMoisATraiter
					   AND
					   (
				    ...
				    */
				    ...
				    select OperID, iID_Convention,vcConventionOperTypeRendement_PCEE_TIN,Rend
				    from (
					   SELECT 
						  OperID = @iID_Oper
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeRendement_PCEE_TIN = @vcConventionOperTypeRendement_PCEE_TIN
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIST,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) -- FT3
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIST,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
					   ) V
				    where V.Rend <> 0
				    /*
				    SELECT 
					   ...
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIST,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
				    HAVING
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIST,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
				    */
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIST,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
...
IF @cEtat = 'S' -- nouveau rendement
    BEGIN
				...
				BEGIN		
				    ...
				    select OperID, iID_Convention,vcConventionOperTypeRendement_ICQ,Rend
				    from (
					   SELECT 
						  OperID = @iID_Oper
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeRendement_ICQ = @vcConventionOperTypeRendement_ICQ
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCBQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) -- FT3
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCBQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
					   ) V
				    where V.Rend <> 0
				    /*
				    SELECT 
					   ...
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCBQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
				    HAVING
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCBQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
				    */
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantCBQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
				    ...
				    select OperID, iID_Convention,vcConventionOperTypeRendement_IMQ,Rend
				    from (
					   SELECT 
						  OperID = @iID_Oper
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeRendement_IMQ = @vcConventionOperTypeRendement_IMQ
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantMMQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) -- FT3
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantMMQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
					   ) V
				    where V.Rend <> 0
				    /*
				    SELECT 
					   ...
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantMMQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
				    HAVING
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantMMQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
				    */

				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantMMQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
...
IF @cEtat = 'S' -- nouveau rendement
    BEGIN
				...
				-- Print '8'
				    ...
				    -- Print '8.1 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    ...
				    select OperID, iID_Convention,vcConventionOperTypeRendement_IIQ,Rend
				    from (
					   SELECT 
						  OperID = @iID_Oper
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeRendement_IIQ = @vcConventionOperTypeRendement_IIQ
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantMIM,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) -- FT3
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantMIM,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
					   ) V
				    where V.Rend <> 0

				    -- Print '8.2 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    /*
				    SELECT 
					   @iID_Oper
					   ,tc.iID_Convention						
					   ,@vcConventionOperTypeRendement_IIQ
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantMIM,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
				    HAVING
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantMIM,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
				    */
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantMIM,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
				    ...
				    from (
					   SELECT 
						  ...
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeRendement_ICQ = @vcConventionOperTypeRendement_ICQ
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantICQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) -- FT3
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantICQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
					   ) V
				    where V.Rend <> 0

				    -- Print '8.3 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    /*
				    SELECT 
					   @iID_Oper
					   ,tc.iID_Convention						
					   ,@vcConventionOperTypeRendement_ICQ
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantICQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
				    HAVING
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantICQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
				    */
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantICQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
				    ...
				    from (
					   SELECT 
						  ...
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeRendement_IMQ = @vcConventionOperTypeRendement_IMQ
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIMQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) -- FT3
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIMQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
					   ) V
				    where V.Rend <> 0

				    -- Print '8.4 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    /*
				    SELECT 
					   @iID_Oper
					   ,tc.iID_Convention						
					   ,@vcConventionOperTypeRendement_IMQ
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIMQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
				    HAVING
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIMQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
				    */
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIMQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
				    ...
				    from (
					   SELECT 
						  ...
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeRendement_IIQ = @vcConventionOperTypeRendement_IIQ
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIIQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) -- FT3
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIIQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
					   ) V
				    where V.Rend <> 0

				    -- Print '8.5 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    /*
				    SELECT 
					   @iID_Oper
					   ,tc.iID_Convention						
					   ,@vcConventionOperTypeRendement_IIQ -- @vcConventionOperTypeRendement_IQI --> Modification du code
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIIQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
				    HAVING
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIIQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
				    */
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIIQ,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
...
-- Print '8.6 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    ...
				    */
				    ...
				    select OperID, iID_Convention,vcConventionOperTypeRendement_III,Rend
				    from (
					   SELECT 
						  OperID = @iID_Oper
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeRendement_III = @vcConventionOperTypeRendement_III
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIII,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
								    +
								    SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIQI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIII,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
								--+
								--SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIQI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
					   ) V
				    where V.Rend <> 0

				    -- Print '8.7 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    /*
				    SELECT 
					   @iID_Oper
					   ,tc.iID_Convention						
					   ,@vcConventionOperTypeRendement_III
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIII,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
					   +
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIQI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))	
				    FROM
					   #tblTransactionConvention tc
				    ...
				    HAVING
					   (SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIII,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
					   +
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIQI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))) <> 0
				    */
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIII,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
																					
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantIQI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
				...
				-- Print '9'
				    ...
				    FROM 
					   ...
					   INNER JOIN dbo.Un_Oper o 
						  ON o.OperID = ct.OperID 
					   --INNER JOIN #tConventionState tmp				ON ccs.ConventionID = tmp.ConventionID AND	ccs.StartDate = tmp.MostRecentDate
					   left JOIN #tRIO rio on rio.iID_Convention_Destination = u.ConventionID --and rio.iID_Convention_Destination is null
				    WHERE 
					   u.ConventionId = ISNULL(@iID_Convention, u.ConventionID)
					   --AND
					   --NOT EXISTS (
				    ...
				    */
				    ...
				    from (
					   SELECT 
						  ...
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeMntSouscrit = @vcConventionOperTypeMntSouscrit
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) -- FT3
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
					   ) V
				    where V.Rend <> 0
				    /*
				    SELECT 
					   ...
					   ,tc.iID_Convention						
					   ,@vcConventionOperTypeMntSouscrit
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
				    HAVING
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
				    */
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
		  ...
		  BEGIN
				...
				FROM 
			     ...
			     WHERE 
					   ...
					   AND cop.ConventionId = ISNULL(@iID_Convention, cop.ConventionID)
				        AND cs.ConventionStateID in ('REE','TRA') -- je veux les convention qui ont cet état
					   AND
					   cop.OperID IN (	SELECT 
										  ISNULL(tx.iID_Operation,-1) 
...
--	RENDEMENT ITR
				...
				--		p.PlanTypeID = 'IND'	-- JFG : 2009-10-30
					   ...
					   and rio.iID_Convention_Source is null--AND c.ConventionID NOT IN (SELECT iID_Convention_Source FROM #tConvSourceRIO WHERE iID_Convention_Source = c.ConventionID) -- FT5
				    GROUP BY 
					   c.ConventionID	
					   ,case when o.OperDate BETWEEN @dtPremierJourDuMoisATraiter and @dtDernierJourDuMoisATraiter then Day(o.OperDate) when o.OperDate < @dtPremierJourDuMoisATraiter then 0 end
				    HAVING 
				    ...
				    */	
				    ...
				    select OperID, iID_Convention,vcConventionOperTypeRendement_TIN,Rend
				    from (
					   SELECT 
						  OperID = @iID_Oper
						  ,iID_Convention = tc.iID_Convention						
						  ,vcConventionOperTypeRendement_TIN = @vcConventionOperTypeRendement_TIN
						  ,Rend = SUM(dbo.fnOPER_CalculerRendement(tc.mMontantITR,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) -- FT3
					   FROM
						  #tblTransactionConvention tc						
					   GROUP BY
						  tc.iID_Convention
					   --HAVING SUM(dbo.fnOPER_CalculerRendement(tc.mMontantITR,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
					   ) V
				    where V.Rend <> 0
				    ...
				    SELECT 
					   ...
					   ,SUM(dbo.fnOPER_CalculerRendement(tc.mMontantITR,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    FROM
					   #tblTransactionConvention tc
				    GROUP BY
					   tc.iID_Convention
				    HAVING
					   SUM(dbo.fnOPER_CalculerRendement(tc.mMontantITR,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1)) <> 0
				    */
				    -- AJOUT DU MONTANT AU MONTANT TOTAL GÉNÉRÉ
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantITR,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
    ...
    -- RENDEMENT SUR REVENUS ACCUMULÉS DES NOUVELLES CONVENTIONS
				    ...
				    FROM
					   ...
					   INNER JOIN dbo.Un_Oper o 
						  ON o.OperID = co.OperID 
					   --INNER JOIN #tConventionState tmp		ON ccs.ConventionID = tmp.ConventionID AND	ccs.StartDate = tmp.MostRecentDate
					   left JOIN #tRIO rio on rio.iID_Convention_Destination = c.ConventionID --and rio.iID_Convention_Destination is null
				    WHERE 
					   c.ConventionId = ISNULL(@iID_Convention, c.ConventionID)
					   --AND
					   --NOT EXISTS (
	   ...
	   IF @vcCode_Rendement = 'REN'
		  ...
		  --	RENDEMENT ITR
				...
				WHERE 
				    cop.ConventionId = ISNULL(@iID_Convention, cop.ConventionID)
				    AND cs.ConventionStateID in ('REE','TRA') -- je veux les convention qui ont cet état
				    AND cop.ConventionOperTypeID = @vcConventionOperTypeRendement_TIN
					   AND
					   cop.OperID IN (	SELECT 
										  ISNULL(tx.iID_Operation,-1) 
									   FROM 
...
IF @cEtat = 'S' -- nouveau rendement
    BEGIN
				...
				BEGIN
				-- Print '12 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    SET @dtDateProspectus = dbo.fnGENE_ObtenirParametre('OPER_DATE_PROSPECTUS', null, null, null, null, null, null)
...
-- ÉPARGNE DES CONVENTIONS INDIVIDUELLES EXISTANTES
				    ...
				    FROM 
					   dbo.Un_Unit u
					   INNER JOIN dbo.Un_Convention co	ON co.ConventionID = u.ConventionID
        				   INNER JOIN dbo.fntCONV_ObtenirStatutConventionEnDate_PourTous(GetDate(), NULL) cs ON co.ConventionID = cs.ConventionID
					   ...
					   --INNER JOIN #tConventionState tmp ON ccs.ConventionID = tmp.ConventionID AND	ccs.StartDate = tmp.MostRecentDate
					   left join #tRIO rio on rio.iID_Convention_Destination = u.ConventionID
				    WHERE p.PlanTypeID = 'IND'
					   ...
					   */
					   AND cs.ConventionStateID IN ('REE','TRA')	-- Type RÉÉÉ ou Transitoire
					   AND	o.OperDate < @dtDernierJourDuMoisATraiter -- modifié par JFA 2009-08-28	
					   AND	(co.dtDateProspectus < @dtDateProspectus OR co.dtDateProspectus IS NULL) --JFA 2009-08-28
				    ...
				    -- Print '12.0.1 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    ...
				    */	
				    --  ****** FIN DU TRAITEMENT POUR LE MOIS À TRAITER *******
				    -- Print '12.1 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)
				    -- CALCUL DU MONTANT ET INSERTION DES TRANSACTIONS DANS LA TABLE DES OPÉRATIONS SUR CONVENTIONS
				    INSERT INTO	dbo.Un_ConventionOper
				    (
					   OperID, ConventionID, ConventionOperTypeID, ConventionOperAmount
				    ...
				    */
				    ...
				    --SET @mMontantTotal_Genere = @mMontantTotal_Genere + ISNULL((SELECT	SUM(dbo.fnOPER_CalculerRendement(tc.mMontantRI,@dTaux_Rendement, @iNbJourMoisATraiter, tc.iJourOperation, @iID_Taux_Rendement, tc.iID_Convention, 1))
				    --															FROM	#tblTransactionConvention tc),0)
				    -- Print '12.2 ' +  LEFT(CONVERT(VARCHAR, getdate(), 120), 30)															
				    IF @bActiveDebug = 1
					   BEGIN
						  IF (SELECT COUNT(*) FROM #tblTransactionConvention) > 0
								BEGIN
...
---------------------------------------------------------------------------------------------------------------------------------
-- 1.3.2.3.10	-- DÉBUT DU CALCUL DU RENDEMENT SUR LES REVENUS ACCUMULÉS SUR L'ÉPARGNE DES CONVENTIONS INDIVIDUELLES EXISTANTES
		  -- QUI NE SONT PAS ISSUES D'UN RIO (OPÉRATION : INM)
---------------------------------------------------------------------------------------------------------------------------------
IF @cEtat = 'S' -- nouveau rendement
    BEGIN
				--Valide si c'est le bon code de rendement pour ce traitement
				IF @vcCode_Rendement = 'RER'
				BEGIN
				    -- Print '13'
				    SET @dtDateProspectus = dbo.fnGENE_ObtenirParametre('OPER_DATE_PROSPECTUS', null, null, null, null, null, null)
				    ...
				    FROM
					   dbo.Un_Convention c
					   INNER JOIN dbo.Un_ConventionOper co					ON c.ConventionID = co.ConventionID
        				   INNER JOIN dbo.fntCONV_ObtenirStatutConventionEnDate_PourTous(GetDate(), NULL) cs ON c.ConventionID = cs.ConventionID
					   INNER JOIN dbo.Un_Plan p 
						  ON p.PlanID = c.PlanID
					   INNER JOIN dbo.Un_Oper o 
				    ...
				    */
				    ...
				    IF @bActiveDebug = 1
					   BEGIN
						  IF (SELECT COUNT(*) FROM #tblTransactionConvention) > 0
								BEGIN
								    SELECT '1.3.2.3.9 NEW : Rendement sur les revenus accumulés sur l''épargne des conventions individuelles existantes non issues d''un RIO', tc.* 
								    FROM #tblTransactionConvention tc
								END
						  ELSE
								BEGIN
								    SELECT '1.3.2.3.9 NEW : Rendement sur les revenus accumulés sur l''épargne des conventions individuelles existantes non issues d''un RIO'
								END
					   END
...
-- REVENUS ACCUMULÉS SUR L'ÉPARGNE DES CONVENTIONS INDIVIDUELLES EXISTANTES
				    ...
				    */
				    ...
				    IF @bActiveDebug = 1
					   BEGIN
						  IF (SELECT COUNT(*) FROM #tblTransactionConvention) > 0
								BEGIN
								    SELECT '1.3.2.3.9 MOD : Rendement sur les revenus accumulés sur l''épargne des conventions individuelles existantes non issues d''un RIO', tc.* 
								    FROM #tblTransactionConvention tc
								END
						  ELSE
								BEGIN
								    SELECT '1.3.2.3.9 MOD : Rendement sur les revenus accumulés sur l''épargne des conventions individuelles existantes non issues d''un RIO'
								END
					   END
    ...
    END
---------------------------------------------------------------------------------------------------------------------------------
		  -- FIN DU CALCUL DU RENDEMENT SUR LES REVENUS ACCUMULÉS SUR L'ÉPARGNE DES CONVENTIONS INDIVIDUELLES EXISTANTES 
		  -- QUI NE SONT PAS ISSUES D'UN RIO (OPÉRATION : INM)
---------------------------------------------------------------------------------------------------------------------------------
				...
				select @mMontantTotal_Genere = isnull(sum( co.ConventionOperAmount),0)
				from Un_ConventionOper co
				where OperID = @iID_OPER

				UPDATE	dbo.tblOPER_TauxRendement
				SET		iID_Operation		= @iID_OPER
					   ,mMontant_Genere	= isnull(@mMontantTotal_Genere,0)
