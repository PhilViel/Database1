...
Valeurs de retours  :    @ReturnValue : > 0 : Réussite
                                <= 0 : Erreurs.
Note                :   
        2006-03-16  Bruno Lapointe          ADX0000847  IA  Création
        2006-05-23  Alain Quirion           ADX0000992  IA  Gestion du type 23 (OUT)
        2006-08-15  Bruno Lapointe          ADX0002063  BR  Optimisation.
        2006-08-18  Bruno Lapointe          ADX0002065  BR  Gestion du champ Un_CESP400.fCotisationGranted
        2007-02-14  Alain Quirion           ADX0001235  IA  Utilisation de dtRegStartDate pour la date de début de régime
        2007-05-23  Bruno Lapointe          ADX0002426  BR  Gestion de la table Un_CESP.
        2007-06-27  Bruno Lapointe          ADX0002502  BR  NAS absent mal géré pour les conventions entrées en vigueur avant le 1 janvier 2003
        ...
        2016-01-12  Steve Picard            Ajouter la déclaration des opérations «PRA» sur les conventions
                                            Pas terminé mais elle va remplacer l'autre IU_UN_CESP400ForOper
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[IU_UN_CESP400ForOper_PRA]
(
...
BEGIN
    ...
    BEGIN
        DECLARE @tConvTrans21 TABLE (
                    ConventionID INTEGER PRIMARY KEY NOT NULL,
                    NAS_Beneficiaire_Cedant VARCHAR(75) NULL,
                    dtDecesBeneficiaire DATE NULL
                )

        INSERT INTO @tConvTrans21 (ConventionID)
        ...
        -- Type 21-1
        IF @tiCESP400WithdrawReasonID = 1
        BEGIN
            INSERT INTO Un_CESP400 (
                   OperID, CotisationID, ConventionID, tiCESP400TypeID, tiCESP400WithdrawReasonID,
                   vcTransID, dtTransaction, iPlanGovRegNumber, ConventionNo, vcSubscriberSINorEN,
                   vcBeneficiarySIN, fCotisation, bCESPDemand, fCESG, fACESGPart,
                   fEAPCESG, fEAP, fPSECotisation, fCLB, fEAPCLB,
                   fPG, fEAPPG, fCotisationGranted 
                )
            SELECT DISTINCT
                   Ct.OperID, Ct.CotisationID, C.ConventionID, @tiCESP400TypeID, @tiCESP400WithdrawReasonID,
                   'FIN', Ct.EffectDate, P.PlanGovernmentRegNo, C.ConventionNo, ISNULL(HS.SocialNumber,''),
                   HB.SocialNumber, 0, C.bCESGRequested,
                        CASE    -- SCEE
                            -- Rembourse rien 0.00 si le solde de subvention et négatif ou à 0.00
                            WHEN ISNULL(G.fCESG + G.fACESG, 0) + ISNULL(C4.fCESG, 0) <= 0 
                                THEN 0
              ...
              FROM Un_Cotisation Ct
                   ...
                   LEFT JOIN ( -- Solde SCEE et SCEE+ et solde de cotisations subventionnées
                            SELECT I.ConventionID,
                                   fCESG = SUM(G.fCESG),    -- SCEE
                                   fACESG = SUM(G.fACESG),  -- SCEE+
                                   fCotisationGranted = SUM(G.fCotisationGranted)   -- Cotisations subventionnées
                              FROM @tConvTrans21 I
                                   JOIN Un_CESP G ON G.ConventionID = I.ConventionID
                             GROUP BY I.ConventionID
                        ) G ON G.ConventionID = C.ConventionID
                   LEFT JOIN ( -- Solde SCEE et SCEE+ à rembourser
                            SELECT I.ConventionID,
                                   fCESG = SUM(C4.fCESG),           -- SCEE
                                   fACESGPart = SUM(C4.fACESGPart), -- SCEE+
                                   fCotisationGranted = SUM(C4.fCotisationGranted)
                              FROM @tConvTrans21 I
                                   JOIN Un_CESP400 C4 ON C4.ConventionID = I.ConventionID
                                   LEFT JOIN Un_CESP900 C9 ON C9.iCESP400ID = C4.iCESP400ID
        ...
        END
        -- Type 21-3
        ELSE IF @tiCESP400WithdrawReasonID = 3
        BEGIN
            INSERT INTO Un_CESP400 (
                   OperID, CotisationID, ConventionID, tiCESP400TypeID, tiCESP400WithdrawReasonID,
                   vcTransID, dtTransaction, iPlanGovRegNumber, ConventionNo, vcSubscriberSINorEN,
            ...
            SELECT DISTINCT
                   ...
                   HB.SocialNumber, 0, C.bCESGRequested,
                        ...
                        -(ISNULL(G.fCESG + G.fACESG,0) + ISNULL(C4.fCESG,0)),   -- SCEE
                        -(ISNULL(G.fACESG,0) + ISNULL(C4.fACESGPart,0)),        -- SCEE+
                    0, 0, 0,
                        -- Rembourse la totalité du BEC
                        -(ISNULL(G.fCLB,0)+ISNULL(C4.fCLB,0)),
                        0,
                    0, 0,
                        -(ISNULL(G.fCotisationGranted,0)+ISNULL(C4.fCotisationGranted,0))   -- Cotisations subventionnées
              FROM Un_Cotisation Ct
                   JOIN dbo.Un_Unit U ON U.UnitID = Ct.UnitID
                   JOIN dbo.Un_Convention C ON C.ConventionID = U.ConventionID
                   JOIN Un_Plan P ON P.PlanID = C.PlanID
                   ...
                   JOIN dbo.Mo_Human HB ON HB.HumanID = B.BeneficiaryID
                   JOIN dbo.Mo_Human HS ON HS.HumanID = C.SubscriberID
                   LEFT JOIN ( -- Solde SCEE, SCEE+, BEC et solde de cotisations subventionnées
                            SELECT I.ConventionID,
                                   fCESG = SUM(G.fCESG),    -- SCEE
                                   fACESG = SUM(G.fACESG),  -- SCEE+
                                   fCLB = SUM(G.fCLB),      -- BEC
             ...
             WHERE Ct.OperID = @OperID
                   ...
                   AND Ct.CotisationID NOT IN (
                            ...
                            SELECT Ct.CotisationID
                              FROM Un_Cotisation Ct
                                   JOIN Un_CESP400 G4 ON G4.CotisationID = Ct.CotisationID
                                                         And G4.tiCESP400TypeID <> 24 /* -- glpi 12174 pour que tout le "not in" n'affecte pas le type 24 */
                                   LEFT JOIN Un_CESP400 R4 ON R4.iReversedCESP400ID = G4.iCESP400ID
                             WHERE Ct.OperID = @OperID
                                   AND G4.iCESP800ID IS NULL -- Pas revenu en erreur
                                   AND G4.iReversedCESP400ID IS NULL -- Pas une annulation
        ...
        END
        -- Type 21-4
        ELSE IF @tiCESP400WithdrawReasonID = 4
        BEGIN
            IF @iTypeRemboursement = 1
            BEGIN
                INSERT INTO Un_CESP400 (
                        OperID, CotisationID, ConventionID, tiCESP400TypeID, tiCESP400WithdrawReasonID,
                        vcTransID, dtTransaction, iPlanGovRegNumber, ConventionNo, vcSubscriberSINorEN,
            ...
            BEGIN
                ...
                SELECT DISTINCT
                       Ct.OperID, Ct.CotisationID, C.ConventionID, @tiCESP400TypeID, @tiCESP400WithdrawReasonID,
                       'FIN', GetDate(), P.PlanGovernmentRegNo, C.ConventionNo, ISNULL(HS.SocialNumber,''),
                       HB.SocialNumber, 0, C.bCESGRequested, 0, 0,
                       0, 0, 0, 
                            -(ISNULL(G.fCLB,0)+ISNULL(C4.fCLB,0)),      -- Rembourse la totalité du BEC
                            0,
                       0, 0, 0
                  FROM Un_Cotisation Ct
                       JOIN dbo.Un_Unit U ON U.UnitID = Ct.UnitID
        ...
        END
        -- Type 21-5
        ELSE IF @tiCESP400WithdrawReasonID = 5  
        BEGIN
            IF @iTypeRemboursement = 1
            BEGIN
                INSERT INTO Un_CESP400 (
            ...
            BEGIN
                ...
                SELECT DISTINCT
                       ...
                       HB.SocialNumber, 0, C.bCESGRequested, 0, 0,
                       0, 0, 0, 
                            -(ISNULL(G.fCLB,0)+ISNULL(C4.fCLB,0)),  -- Remboursement total du BEC
                            0,
                       0, 0, 0
                  FROM Un_Cotisation Ct
                       JOIN dbo.Un_Unit U ON U.UnitID = Ct.UnitID
            ...
            ELSE IF @iTypeRemboursement = 3
            BEGIN
                -- GLPI 5323 DHuppé(2011-04-06) : au lieu de prnedre la date de la veille, on prend la date du lendemain pour être certain d'avoir le dernier changement de bénéficiaire
                DECLARE @dtDate DATETIME = DATEADD(dd,+1,GETDATE())

                UPDATE T
                    SET NAS_Beneficiaire_Cedant = HU.SocialNumber,
                        dtDecesBeneficiaire = HU.deathdate
                    FROM @tConvTrans21 T
                        CROSS APPLY dbo.fntCONV_RechercherChangementsBeneficiaire(NULL, NULL, T.ConventionID, NULL,  @dtDate, NULL, 1, NULL, NULL, NULL, NULL, NULL, NULL) fnt
			         INNER JOIN dbo.Mo_Human hu ON hu.HumanID = fnt.iID_Ancien_Beneficiaire

                INSERT INTO Un_CESP400 (
                        OperID, CotisationID, ConventionID, tiCESP400TypeID, tiCESP400WithdrawReasonID,
                        vcTransID, dtTransaction, iPlanGovRegNumber, ConventionNo, vcSubscriberSINorEN,
                        vcBeneficiarySIN,   -- à modifier.
                            fCotisation, bCESPDemand, fCESG, fACESGPart,
                        fEAPCESG, fEAP, fPSECotisation, fCLB, fEAPCLB,
                        fPG, fEAPPG, fCotisationGranted 
                    )
                SELECT DISTINCT
                       Ct.OperID, Ct.CotisationID, C.ConventionID, @tiCESP400TypeID, @tiCESP400WithdrawReasonID,
                       'FIN', Ct.EffectDate, P.PlanGovernmentRegNo, C.ConventionNo, ISNULL(HS.SocialNumber,''),
                       (SELECT NAS_Beneficiaire_Cedant FROM @tConvTrans21 WHERE ConventionID = C.ConventionID), 
                            0, C.bCESGRequested,
                            -(ISNULL(G.fCESG + G.fACESG, 0) + ISNULL(C4.fCESG, 0)),     -- SCEE
                            -(ISNULL(G.fACESG, 0) + ISNULL(C4.fACESGPart, 0)),          -- SCEE+
                       0, 0, 0, 0, 0,
                       0, 0, 
                            -(ISNULL(G.fCotisationGranted,0)+ISNULL(C4.fCotisationGranted,0))   -- Cotisation
                  FROM Un_Cotisation Ct
                       JOIN dbo.Un_Unit U ON U.UnitID = Ct.UnitID
                       JOIN dbo.Un_Convention C ON C.ConventionID = U.ConventionID
                       JOIN Un_Plan P ON P.PlanID = C.PlanID
        ...
        END
        -- Type 21-7
        ELSE IF @tiCESP400WithdrawReasonID = 7
        BEGIN
            INSERT INTO Un_CESP400 (
                    OperID, CotisationID, ConventionID, tiCESP400TypeID, tiCESP400WithdrawReasonID,
                    vcTransID, dtTransaction, iPlanGovRegNumber, ConventionNo, vcSubscriberSINorEN,
            ...
            SELECT DISTINCT
                    ...
                    'FIN', Ct.EffectDate, P.PlanGovernmentRegNo, C.ConventionNo, ISNULL(HS.SocialNumber,''),
                    HB.SocialNumber, 0, C.bCESGRequested, 
                        -(ISNULL(G.fCESG + G.fACESG,0) + ISNULL(C4.fCESG,0)),   -- Rembourse la totalité du SCEE
                        -(ISNULL(G.fACESG,0) + ISNULL(C4.fACESGPart,0)),        -- Rembourse la totalité du SCEE+
                    0, 0, 0, 
                        -(ISNULL(G.fCLB,0)+ISNULL(C4.fCLB,0)),                  -- Rembourse la totalité du BEC
                        0,
                    0, 0,
        ...
        IF @tiCESP400WithdrawReasonID IN (/*1, 3,*/ 7)
        BEGIN 
            SELECT @iID_Convention = co.ConventionID,
                   @bBECPresent = CASE @tiCESP400WithdrawReasonID
                                      WHEN 1 THEN dbo.fnPCEE_ValiderPresenceBEC(co.ConventionID)
                                      WHEN 3 THEN c.bCLBRequested
                                      WHEN 7 THEN dbo.fnPCEE_ValiderPresenceBEC(co.ConventionID)
                                  END,
                   @iID_Beneficiaire = c.BeneficiaryID
              FROM dbo.Un_ConventionOper co
                   INNER JOIN dbo.Un_Convention c ON co.ConventionID = c.ConventionID
             WHERE co.OperID = @OperID
        
            IF @bBECPresent = 1 
            BEGIN
                -- Récupérer la somme du BEC
                SELECT @mCLB = SUM(ce.fCLB)
                  FROM @tConvTrans21 t
                       INNER JOIN dbo.Un_CESP ce ON t.ConventionID = ce.ConventionID
                 GROUP BY t.ConventionID         

                IF @tiCESP400WithdrawReasonID IN (1, 7)
                BEGIN      
                    ...
                    INSERT INTO @tConvention (
                            iID_Convention, iID_Souscripteur, iID_Beneficiaire, vcConventionNO,
                            cID_PlanType, vcPlanDesc, bPresenceBec, dtDateEntreeVigueur
                        )
                    SELECT fnt.iConventionID, fnt.iSubscriberID, fnt.iBeneficiaryID, fnt.vcConventionNO,
                           fnt.cPlanTypeID, fnt.vcPlanDesc, 
                                bPresenceBec = dbo.fnPCEE_ValiderPresenceBEC(fnt.iConventionID),
                                u.InForceDate
                      FROM dbo.fntCONV_ObtenirListeConventionsParBeneficiaire(GETDATE(), @iID_Beneficiaire) fnt    -- LISTE DES CONVENTION REE ACTIVES
                           INNER JOIN dbo.Un_Unit u ON u.ConventionID = fnt.iConventionID
                     WHERE fnt.iConventionID <> @iID_Convention
                    ...
                    SET @iID_ConventionSuggere = (SELECT TOP 1 t.iID_Convention FROM @tConvention t 
                                                   WHERE t.cID_PlanType = 'IND' ORDER BY t.dtDateEntreeVigueur ASC, t.iID_Convention)

                    IF @iID_ConventionSuggere IS NULL
                        SET @iID_ConventionSuggere = (SELECT TOP 1 t.iID_Convention    FROM @tConvention t 
                                                       WHERE t.vcPlanDesc = 'Reeeflex' ORDER BY t.dtDateEntreeVigueur ASC, t.iID_Convention)

                    IF @iID_ConventionSuggere IS NULL
                        SET @iID_Convention = (SELECT TOP 1 t.iID_Convention       FROM @tConvention t 
                                                WHERE t.vcPlanDesc = 'Universitas' ORDER BY t.dtDateEntreeVigueur ASC, t.iID_Convention)                

                    IF @iID_ConventionSuggere IS NOT NULL    -- On transfère le BEC
                        EXECUTE @iRetour = dbo.psPCEE_CreerDemandeBec @iID_ConventionSuggere        
                END
                ELSE IF @tiCESP400WithdrawReasonID = 3
                   SELECT @mCLB = @mCLB + SUM(C4.fCLB) -- Solde de BEC à rembourser
                     FROM @tConvTrans21 I
                          JOIN Un_CESP400 C4 ON C4.ConventionID = I.ConventionID
                          LEFT JOIN Un_CESP900 C9 ON C9.iCESP400ID = C4.iCESP400ID
                          LEFT JOIN Un_CESP CE ON CE.OperID = C4.OperID
    ...
    -- 21-8     --    
    -----------
    IF @tiCESP400TypeID = 21 AND @tiCESP400WithdrawReasonID = 8
    BEGIN
    ...
    -- Type 21-9 --
    ---------------
    IF @tiCESP400TypeID = 21 AND @tiCESP400WithdrawReasonID = 9 AND @iTypeRemboursement = 1        -- 2009-11-11 : JFG
    BEGIN
    ...
    -- Type 21-9 BEC --
    ---------------
    IF @tiCESP400TypeID = 21 AND @tiCESP400WithdrawReasonID = 9 AND @iTypeRemboursement = 2        -- 2009-11-11 : JFG
    BEGIN
    ...
    */
    ...
    -- Type 21-10 --
    ---------------
    IF @tiCESP400TypeID = 21 AND @tiCESP400WithdrawReasonID = 10
    BEGIN
    ...
    -- Type 21-11 --
    ---------------
    IF @tiCESP400TypeID = 21 AND @tiCESP400WithdrawReasonID = 11 AND @iTypeRemboursement = 1        -- 2009-11-11 : JFG
    BEGIN
    ...
    -- Type 21-11 BEC --
    -------------------        
    IF @tiCESP400TypeID = 21 AND @tiCESP400WithdrawReasonID = 11 AND @iTypeRemboursement = 2
    BEGIN
