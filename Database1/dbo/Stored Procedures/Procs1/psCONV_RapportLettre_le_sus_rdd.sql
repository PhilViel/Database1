...
Code du service		: psCONV_RapportLettre_le_sus_rdd
...
Exemple d’appel		:	EXEC psCONV_RapportLettre_le_sus_rdd 'U-20011219006,T-20140501001'
						EXEC psCONV_RapportLettre_le_sus_rdd 'X-20110406006'
						EXEC psCONV_RapportLettre_le_sus_rdd 'T-201111011164'
drop procedure psCONV_RapportLettre_le_sus_rdd
...
****************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportLettre_le_sus_rdd] @cConventionno varchar(max)
AS
BEGIN
	DECLARE
		@today datetime,
	...
	SELECT distinct
		...
		PlanClassification = [dbo].[fnCONV_ObtenirDossierClient](c.SubscriberID,1) + '\' + replace(LEFT(CONVERT(VARCHAR, @today, 120), 10),'-','')
							+ case HS.langID when 'FRA' then '_le_sus_rdd' when 'ENU' then '_le_sus_rdd_ang' end
