...
Code du service		: psCONV_RapportLettre_le_compte_invalide_ferme (comme le_2_NSF)
Nom du service		: Générer la lettre d'arrêt de paiement "le_compte_invalide_ferme"
But 				: 
Facette				: CONV
...
EXEC psCONV_RapportLettre_le_compte_invalide_ferme 809999, 242.21
...
Historique des modifications:
		...
		------------	----------------------------------	-----------------------------------------	------------
		2018-08-28		Donald Huppé						Création du service	
		
****************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportLettre_le_compte_invalide_ferme] 
	@SubscriberID INT,
	@MontantAverser MONEY
AS
BEGIN
	DECLARE
		@today datetime,
	...
	SELECT 
		...
		MontantPrelevement = @MontantAverser, --SUM(V.MontantPrelevement) + SUM(V.INC),
	...
	SELECT distinct
		...
		PlanClassification = [dbo].[fnCONV_ObtenirDossierClient](c.SubscriberID,1) + '\' + replace(LEFT(CONVERT(VARCHAR, @today, 120), 10),'-','')
							+ case HS.langID when 'FRA' then '_le_compte_invalide_ferme' when 'ENU' then '_le_compte_invalide_ferme_ang' end
