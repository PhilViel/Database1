...
Code de service		:		psOPER_ObtenirLettreConfirmationRIOSpecial
Nom du service		:		Obtenir Lettre de confirmation RIO Special
...
Historique des modifications :
						...
						2018-11-08					Maxime Martel							Utilisation de planDesc_ENU et regroupement de regime de la table plan
                        2018-11-12                  Pierre-Luc Simard                       N'est plus utilisée
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[psOPER_ObtenirLettreConfirmationRIOSpecial](
	...
	*/
	...
	INSERT INTO #Letter
		SELECT 
			...
			-- FT1
			OperAmount = dbo.fn_Mo_MoneyToStr(ABS(CO.Cotisation), SUB.LangID, 0),			
		...
		JOIN (
			SELECT 
				 UN.UnitID
				,Cotisation = SUM(Cotisation) --CT.*
			FROM dbo.Un_Unit UN
			JOIN dbo.Un_Convention CN ON CN.ConventionID = UN.ConventionID
			JOIN Un_Cotisation CT ON CT.UnitID = UN.UnitID
			--WHERE CN.ConventionNo =  'T-20110331001' -- 'R-20061214035'
			GROUP BY UN.UnitID
			) CO ON CO.UnitID = UDest.UnitID 
	...
	BEGIN
		...
		INSERT INTO CRQ_Doc (DocTemplateID, DocOrderConnectID, DocOrderTime, DocGroup1, DocGroup2, DocGroup3, Doc)
			SELECT 
				...
				ISNULL(RepresentantFirstName,'')+';'+
				ISNULL(RepresentantLastName,'')+';'+
				ISNULL(PlanDesc,'')+';'+
				ISNULL(Age,'')+';'
			FROM #Letter
