...
Fonction: 12 heures
	...
	EXECUTE psCONV_RapportLettre_le_adm_PAE_2017 @NoEnvoi = 1
	EXECUTE psCONV_RapportLettre_le_adm_PAE_2017 @NoEnvoi = 2
	EXECUTE psCONV_RapportLettre_le_adm_PAE_2017 @NoEnvoi = 3
	EXECUTE psCONV_RapportLettre_le_adm_PAE_2017 @NoEnvoi = 4
	EXECUTE psCONV_RapportLettre_le_adm_PAE_2017 @NoEnvoi = 5
	EXECUTE psCONV_RapportLettre_le_adm_PAE_2017 @NoEnvoi = 6
	EXECUTE psCONV_RapportLettre_le_adm_PAE_2017 -1 -- non résident
...
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportLettre_le_adm_PAE_2017] 
(
...
/*
1er envoi : bénéficiaires admissibles au 2e PAE (ayant retiré un PAE dans les deux dernières années, bénéficiaires de moins de 25 ans) LISTE 1
2e envoi : bénéficiaires admissibles au 3e PAE (ayant retiré un PAE dans les deux dernières années, bénéficiaires de moins de 25 ans) LISTE 2
3e envoi : bénéficiaires admissibles au 1er PAE (nouvelle cohorte 2017 seulement, celle de 17 ans d'âge du bénéficiaire) LISTE 3
4e envoi : bénéficiaires admissibles au 1er PAE (anciennes cohortes (2016 et moins) mais moins de 25 ans d’âge du bénéficiaire) LISTE 4
5e envoi : bénéficiaires admissibles au 2e PAE (n’ayant pas retiré un PAE dans les deux dernières années mais moins de 25 ans du bénéficiaire) LISTE 5
6e envoi : bénéficiaires admissibles au 3e PAE (n’ayant pas retiré un PAE dans les deux dernières années mais moins de 25 ans du bénéficiaire) LISTE 6
...
*/
	...
	IF @NoEnvoi = 1 
		BEGIN
			...
			where 1=1
				...
				--and hb.BirthDate BETWEEN '1992-01-01' and '2000-01-01'
				and hb.BirthDate BETWEEN '1993-01-01' and '2001-01-01'
	...
	SELECT 
		...
		RegimeBilingue = CASE 
					WHEN ISNULL(SH.LangID, 'FRA') = 'FRA' THEN
						UPPER (GC.vcDescription)
					ELSE	
						
						CASE GC.vcDescription 
						WHEN 'Reeeflex' THEN 'REFLEX' 
						WHEN 'Individuel' THEN 'INDIVIDUAL' 
						ELSE UPPER(GC.vcDescription) 
						END
		...
		RegionSousc = case 
					when ADS.StateName = 'QC' then 'QC'
					when ADS.StateName = 'NB' then 'NB'
					WHEN ads.CountryID = 'CAN' AND ADS.StateName not in ('QC','NB') then 'National'
					WHEN ads.CountryID <> 'CAN'  then 'International'
					END,

		AdresseSouscPerdue = su.AddressLost,

		CourrielSousc = case WHEN dbo.fnGENE_ValidateEmail(
										isnull(SCRm.vcCourriel,isnull(SCRt.vcCourriel,SCRa.vcCourriel))
										) = 1
									THEN isnull(SCRm.vcCourriel,isnull(SCRt.vcCourriel,SCRa.vcCourriel))
							ELSE NULL
							END,

		CourrielBenef = case WHEN dbo.fnGENE_ValidateEmail(
										isnull(CRm.vcCourriel,isnull(CRt.vcCourriel,CRa.vcCourriel))
										) = 1
									THEN isnull(CRm.vcCourriel,isnull(CRt.vcCourriel,CRa.vcCourriel))
							ELSE NULL
							END,
	...
	LEFT JOIN tblGENE_Courriel CRm on CRm.iID_Source = B.BeneficiaryID AND GETDATE() BETWEEN CRm.dtDate_Debut and ISNULL(CRm.dtDate_Fin,'9999-12-31') and CRm.iID_Type = 1 and CRm.bInvalide = 0
	LEFT JOIN tblGENE_Courriel CRt on CRt.iID_Source = B.BeneficiaryID AND GETDATE() BETWEEN CRt.dtDate_Debut and ISNULL(CRt.dtDate_Fin,'9999-12-31') and CRt.iID_Type = 2 and CRt.bInvalide = 0
	LEFT JOIN tblGENE_Courriel CRa on CRa.iID_Source = B.BeneficiaryID AND GETDATE() BETWEEN CRa.dtDate_Debut and ISNULL(CRa.dtDate_Fin,'9999-12-31') and CRa.iID_Type = 4 and CRa.bInvalide = 0

	LEFT JOIN tblGENE_Courriel SCRm on SCRm.iID_Source = SU.SubscriberID AND GETDATE() BETWEEN SCRm.dtDate_Debut and ISNULL(SCRm.dtDate_Fin,'9999-12-31') and SCRm.iID_Type = 1 and SCRm.bInvalide = 0
	LEFT JOIN tblGENE_Courriel SCRt on SCRt.iID_Source = SU.SubscriberID AND GETDATE() BETWEEN SCRt.dtDate_Debut and ISNULL(SCRt.dtDate_Fin,'9999-12-31') and SCRt.iID_Type = 2 and SCRt.bInvalide = 0
	LEFT JOIN tblGENE_Courriel SCRa on SCRa.iID_Source = SU.SubscriberID AND GETDATE() BETWEEN SCRa.dtDate_Debut and ISNULL(SCRa.dtDate_Fin,'9999-12-31') and SCRa.iID_Type = 4 and SCRa.bInvalide = 0
