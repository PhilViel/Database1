...
Code du service		: psCONV_RapportLettre_le_c_red_chq_ajout_mens
...
Exemple d’appel		:	
			EXEC psCONV_RapportLettre_le_c_red_chq_ajout_mens 'X-20101203018,U-20110418005'
						
drop procedure psCONV_RapportLettre_le_c_red_chq_ajout_mens
...
****************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportLettre_le_c_red_chq_ajout_mens] @cConventionno varchar(max)
AS
BEGIN
	DECLARE
		@today datetime,
	...
	SELECT distinct
		...
		PlanClassification = [dbo].[fnCONV_ObtenirDossierClient](c.SubscriberID,1) + '\' + replace(LEFT(CONVERT(VARCHAR, @today, 120), 10),'-','')
							+ case HS.langID when 'FRA' then '_le_c_red_chq_ajout_mens' when 'ENU' then '_le_c_red_chq_ajout_mens_ang' end
