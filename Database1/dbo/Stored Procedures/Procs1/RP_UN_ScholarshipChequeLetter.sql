...
Nom                 :	RP_UN_ScholarshipChequeLetter
Description         :	Rapport de fusion word des lettres de transmission de chèque de bourses
Valeurs de retours  :	Dataset de données
Note                :							
									2004-06-02	Bruno Lapointe		Création
					ADX0001398	BR	2005-04-18	Bruno Lapointe		Correction du bug
					ADX0000704	IA	2005-07-05	Bruno Lapointe		Modifier la lettre pour que dans le troisième
																	champ d’information, qui contenait le numéro de bourse, est aussi le nom
																	et prénom du souscripteur.  Donc on avait « 1 » et maintenant on a
																	« 1 – Ranger, Éric ».  Cela permet de trier les documents en ordre de 
																	numéro de bourse, nom et prénom du souscripteur pour l’impression dans
...
-- EXEC RP_UN_ScholarshipChequeLetter 1, 175358, 1 
SELECT ConventionID 
FROM Un_Convention 
WHERE ConventionNo = 'D-20000802002'

*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[RP_UN_ScholarshipChequeLetter] (
...
BEGIN
	...
	CREATE TABLE #Letter(
		...
		ScholarshipAmount VARCHAR(125),
		ChequeAmount VARCHAR(75), 
		AdvanceAmountTXT VARCHAR(250),
	...
	-- Trouve la bourse payée la plus grande 
	SELECT
		@ScholarshipID = MAX(ScholarshipID)
	FROM Un_Scholarship
	WHERE ConventionID = @ConventionID
	  AND ScholarshipStatusID = 'PAD'
	...
	FROM Un_Scholarship
	WHERE ConventionID = @ConventionID
	  AND ScholarshipStatusID IN ('RES', 'TPA', 'ADM', 'WAI', 'PAD')
	  AND YearDeleted = 0
	...
	INSERT INTO #Letter
		...
		SELECT
			...
			ScholarshipNoLast = 
				CASE 
					WHEN @LastScholarshipID = S.ScholarshipID THEN 
						CASE BH.LangID
		...
		JOIN (
			...
			WHERE SP.ScholarshipID = @ScholarshipID
			GROUP BY L.OperID
		...
		LEFT JOIN (
			...
			WHERE C.ConventionOperTypeID = 'AVC'
			  AND P.ScholarshipID = @ScholarshipID
			) AV ON (AV.OperID = O.OperID)
		LEFT JOIN (
			SELECT
				OperID = MIN(O.OperID),
			...
			JOIN Un_ScholarshipPmt P ON P.OperID = O.OperID
			WHERE P.ScholarshipID = @ScholarshipID
