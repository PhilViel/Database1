...
Code du service :   psIQEE_CreerTransactions06_31
Nom du service  :   Créer les transactions de  type 06, sous type 31 - Paiement versé à un établissement d'enseignement
But             :   Sélectionner, valider et créer les transactions de type 06 – Impôt spécial, 31 - PEE, 
...
Exemple d’appel :   
        exec dbo.psIQEE_CreerTransactions06_31 10, 0, NULL, 0, 'T',0
...
Historique des modifications:
    ...
    ----------  --------------------    --------------------------------------------------------------------------
    2017-10-22  Steeve Picard           Création du service pour les PEE (Basé sur dbo.psIQEE_CreerTransactions06_41) 
    2017-11-15  Steeve Picard           Remplacement de la view «vwOPER_Active» par la fonction «fntOPER_Active»
    ...
    2018-04-01  Steeve Picard           Récupère les soldes en date du jour au lieu de la «@dtFinCotisation»
...
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[psIQEE_CreerTransactions06_31]
(
...
BEGIN
    ...
    PRINT ''
    PRINT 'Déclaration des paiements versés à un établissement d''enseignement (T06-31) pour ' + STR(@siAnnee_Fiscale, 4)
    PRINT '--------------------------------------------------------------------------------------'
    PRINT '   ' + Convert(varchar(20), GetDate(), 120) + ' EXEC psIQEE_CreerTransactions06_31 started'

    -- Empêcher ces déclarations en PROD
    IF @siAnnee_Fiscale < 2018 AND @bit_CasSpecial = 0 --AND @@SERVERNAME IN ('SRVSQL12', 'SRVSQL25')
    BEGIN
        PRINT '   *** Déclaration non-implanté en PROD ou avant 2017'
        --RETURN
    END 
    ...
    DECLARE @StartTimer datetime = GetDate(),           @QueryTimer datetime,                           
            @ElapseTime datetime,                       @MaxRow INT = 0,
    ...
    PRINT '   ' + Convert(varchar(20), GetDate(), 120) + ' Identifie les conventions devant déclarer une T06-31'
    BEGIN
        ------------------------------------------------------------------------------------------------------------
        -- Identifier et sélectionner les conventions ayant eu un PEE au contrat ou d’un dossier bénéficiaire
        ------------------------------------------------------------------------------------------------------------
        IF OBJECT_ID('tempdb..#TB_Convention_06_31') IS NOT NULL
            DROP TABLE #TB_Convention_06_31

        PRINT '   ' + Convert(varchar(20), GetDate(), 120) + '   - Récupère les conventions ayant eu un «PEE» au contrat'
        SET @QueryTimer = GetDate()
    
        SET ROWCOUNT @MaxRow
    
        ;WITH CTE_Convention_PEE as (
            SELECT DISTINCT
                O.OperID, O.OperDate, 
                ConventionID = COALESCE(C.ConventionID, CO.ConventionID, Ct.ConventionID)
            FROM 
                dbo.fntOPER_Active(@dtDebutCotisation, @dtFinCotisation) O
                LEFT JOIN dbo.Un_ConventionOper CO ON CO.OperID = O.OperID
                LEFT JOIN dbo.Un_CESP C ON C.OperID = O.OperID
                LEFT JOIN (
                    SELECT DISTINCT Ct.OperID, U.ConventionID
                      FROM dbo.Un_Cotisation Ct
                           JOIN dbo.Un_Unit U ON U.UnitID = Ct.UnitID
                ) Ct ON Ct.OperID = O.OperID
            WHERE 
                O.OperTypeID = 'PEE' 
                AND COALESCE(C.ConventionID, CO.ConventionID, Ct.ConventionID) IS NOT NULL
        ),
        CTE_Convention as (
            SELECT 
        ...
        INTO #TB_Convention_06_31
        FROM
            CTE_Convention C
            LEFT JOIN dbo.fntIQEE_CalculerSoldeIQEE_ParConvention(NULL, GETDATE(), 0) S ON S.ConventionID = C.ConventionID
            LEFT JOIN (
                SELECT I.iID_Convention, I.tiCode_Version, I.cStatut_Reponse, 
                       RowNum = ROW_NUMBER() OVER(PARTITION BY I.iID_Convention ORDER BY F.dtDate_Creation DESC, I.dtDate_Evenement DESC)
                  FROM dbo.tblIQEE_ImpotsSpeciaux I
                       ...
                       JOIN @TB_FichierIQEE F ON F.iID_Fichier_IQEE = R.iID_Fichier_IQEE
                 WHERE I.siAnnee_Fiscale = @siAnnee_Fiscale
                       AND T.cCode_Type_SousType = '06-31'
                       AND cStatut_Reponse IN ('E', 'X')
            ) DIS ON DIS.iID_Convention = C.ConventionID AND DIS.RowNum = 1 AND DIS.tiCode_Version <> 1 AND DIS.cStatut_Reponse IN ('A','R')
            LEFT JOIN (
                SELECT I.iID_Convention, I.tiCode_Version, I.cStatut_Reponse, 
                       RowNum = ROW_NUMBER() OVER(PARTITION BY I.iID_Convention ORDER BY I.dtDate_Evenement DESC, I.iID_Impot_Special DESC)
                  FROM dbo.tblIQEE_ImpotsSpeciaux I 
                       JOIN dbo.vwIQEE_Enregistrement_TypeEtSousType T ON T.iID_Sous_Type = I.iID_Sous_Type
                 WHERE T.cCode_Type_SousType = '06-91'
                       AND cStatut_Reponse IN ('E', 'X')
            ) FRM ON FRM.iID_Convention = C.ConventionID AND FRM.RowNum = 1 AND FRM.tiCode_Version <> 1 AND FRM.cStatut_Reponse  ='R'
        WHERE
            DIS.iID_Convention IS NULL
            AND FRM.iID_Convention IS NULL
...
--        SELECT *, @dtFinCotisation FROM #TB_Convention_06_31 

        SET ROWCOUNT 0

        IF NOT EXISTS(SELECT TOP 1 * FROM #TB_Convention_06_31)
           RETURN

        IF @iCount < 5
            SET @MaxRow = @iCount

        IF Object_ID('tempDB..#TB_Beneficiary_06_31') IS NOT NULL
            DROP TABLE #TB_Beneficiary_06_31
        ...
        IF @IsDebug <> 0 AND @MaxRow between 1 and 10
            select * 
            from #TB_Convention_06_31 TB JOIN #TB_Beneficiary_06_31 B ON B.iID_Convention = TB.ConventionID
            ORDER BY TB.ConventionNo
    END

    --------------------------------------------------------------------------------------------------
    -- Valider les «PEE» des convention et conserver les raisons de rejet en vertu des validations
    --------------------------------------------------------------------------------------------------
    ...
    BEGIN
        ...
        IF OBJECT_ID('tempdb..#TB_Validation_06_31') IS NOT NULL
            DROP TABLE #TB_Validation_06_31
        ...
        WHILE Exists(SELECT TOP 1 * FROM #TB_Validation_06_31 WHERE iOrdre_Presentation > @iOrdre_Presentation) 
        BEGIN
            SET @iCountRejets = 0
            ...
            BEGIN TRY
                -- Validation : L'impôt spécial de «PEE» de convention a déjà été envoyé et une réponse reçue de RQ
                IF @iCode_Validation = 1301 
                BEGIN
                    ; WITH CTE_Convention as (
                        SELECT
                            C.ConventionID, C.StatutID
                        FROM
                            #TB_Convention_06_31 C
                        WHERE
                            Exists (
                ...
                -- Validation : L'impôt spécial de «PEE» de convention est en cours de traitement par RQ et est en attente d’une réponse de Revenu Québec
                IF @iCode_Validation = 1302 
                BEGIN
                    ; WITH CTE_Convention as (
                        SELECT
                            C.ConventionID, C.StatutID
                ...
                -- Validation : Une erreur soulevée par Revenu Québec est en cours de traitement pour l'impôt spécial de paiement de revenus accumulés (PEE)
                IF @iCode_Validation = 1303 
                BEGIN
                    ; WITH CTE_Convention as (
                        SELECT
                            C.ConventionID, C.StatutID
                ...
                -- Validation : Le NAS du bénéficiaire relié au «PEE» de convention est absent ou invalide
                IF @iCode_Validation = 1304 
                BEGIN
                    ; WITH CTE_Convention as (
                        SELECT
                        ...
                        FROM
                            #TB_Convention_06_31 C
                            JOIN #TB_Beneficiary_06_31 B ON B.iID_Convention = C.ConventionID
                        WHERE
                ...
                -- Validation : Le nom du bénéficiaire relié au «PEE» de convention est absent
                IF @iCode_Validation = 1305 
                BEGIN
                    ; WITH CTE_Convention as (
                        SELECT
                ...
                -- Validation : Le prénom du bénéficiaire relié au «PEE» de convention est absent
                IF @iCode_Validation = 1306
                BEGIN
                    ; WITH CTE_Convention as (
                        SELECT
                ...
                -- Validation : La date de naissance du bénéficiaire reliée au «PEE» de convention est absente
                IF @iCode_Validation = 1307
                BEGIN
                    ; WITH CTE_Convention as (
                        SELECT
                ...
                -- Validation : La date de naissance du bénéficiaire reliée au «PEE» de convention est invalide
                IF @iCode_Validation = 1308
                BEGIN
                    ; WITH CTE_Convention as (
                        SELECT
                ...
                -- Validation : Le sexe du bénéficiaire relié au «PEE» de convention n’est pas défini
                IF @iCode_Validation = 1309
                BEGIN
                    ; WITH CTE_Convention as (
                        SELECT
                ...
                -- Validation : Le prénom du bénéficiaire relié au «PEE» de convention contient au moins 1 caractère non conforme
                IF @iCode_Validation = 1310
                BEGIN
                    ; WITH CTE_Convention as (
                        SELECT
                ...
                -- Validation : Le nom du bénéficiaire relié au «PEE» de convention contient au moins 1 caractère non conforme
                IF @iCode_Validation = 1311
                BEGIN
                    ; WITH CTE_Convention as (
                        SELECT
                ...
                -- Validation : Les transferts doivent être déclarés à RQ avant que celle-ci puisse être déclarée
                IF @iCode_Validation = 1315
                BEGIN
                    INSERT INTO #TB_Rejets_06_31 (
                        iID_Convention, iID_Validation, vcDescription,
                        vcValeur_Reference, vcValeur_Erreur, iID_Lien_Vers_Erreur_1, iID_Lien_Vers_Erreur_2, iID_Lien_Vers_Erreur_3
                    ...
                    FROM
                        ...
                        JOIN #TB_Convention_06_31 TB ON TB.ConventionID = T.ConventionID
                    WHERE
                ...
                -- Validation : Les PAE doivent être déclarés à RQ avant que celle-ci puisse être déclarée
                IF @iCode_Validation = 1316
                BEGIN
                    INSERT INTO #TB_Rejets_06_31 (
                        iID_Convention, iID_Validation, vcDescription,
                        vcValeur_Reference, vcValeur_Erreur, iID_Lien_Vers_Erreur_1, iID_Lien_Vers_Erreur_2, iID_Lien_Vers_Erreur_3
                    ...
                    SELECT DISTINCT
                        ...
                        NULL, NULL, TB.StatutID, NULL, NULL
                    FROM
                        dbo.fntIQEE_PaiementBeneficiaire_NonDeclare(DEFAULT, @dtFinCotisation) PB
                        JOIN #TB_Convention_06_31 TB ON TB.ConventionID = PB.ConventionID
                    WHERE
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    DELETE FROM #TB_Convention_06_31
                    WHERE EXISTS (SELECT * FROM #TB_Rejets_06_31 WHERE iID_Convention = ConventionID AND iID_Validation = @iID_Validation)
                END
            END
        ...
        DELETE FROM C
          FROM #TB_Convention_06_31 C
               JOIN #TB_Rejets_06_31 R ON R.iID_Convention = C.ConventionID
    ...
    PRINT '   ' + Convert(varchar(20), GetDate(), 120) + ' EXEC psIQEE_CreerTransactions06_31 completed' 
                + ' (Elapsed time ' + LTrim(Str((DatePart(day, @ElapseTime)-1) * 24 + DatePart(hour, @ElapseTime),4)) + Right(CONVERT(VARCHAR, @ElapseTime, 120), 6) + ')'

    IF OBJECT_ID('tempdb..#TB_Validation_06_31') IS NOT NULL
        DROP TABLE #TB_Validation_06_31
    IF OBJECT_ID('tempdb..#TB_Rejets_06_31') IS NOT NULL
        DROP TABLE #TB_Rejets_06_31
    IF OBJECT_ID('tempdb..#TB_Beneficiary_06_31') IS NOT NULL
        DROP TABLE #TB_Beneficiary_06_31
    IF OBJECT_ID('tempdb..#TB_Convention_06_31') IS NOT NULL
        DROP TABLE #TB_Convention_06_31
