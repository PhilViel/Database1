...
select count(*) from tmpReleveCompteAllConv

select count(*) from tmpReleveCompteDTL where WhatToInsert = '20-DTL-4'

-------------------------------- ###################### VÉRIFIER VERSION DE PS 
-------------------------------- ###################### VÉRIFIER VERSION DE PS 
-------------------------------- ###################### VÉRIFIER VERSION DE PS 
-------------------------------- ###################### VÉRIFIER VERSION DE PS 

exec psCONV_ReleveDeCompteDatasetGlobal 'tmpReleveCompteAllConv' -- DROP TABLE tmpReleveCompteAllConv -- 
create clustered index ClsIndex_tmpReleveCompteAllConv_ConventionID on tmpReleveCompteAllConv(ConventionID) 
					
---------------------------
exec psCONV_ReleveDeCompteDatasetGlobal 'AnneeQualification'


exec psCONV_ReleveDeCompteDatasetGlobal 'ConventionRCinfo' -- pas long, moins de 5 minutes -- drop table ConventionRCinfo
create clustered index ClsIndex_ConventionRCinfo_ConventionID on ConventionRCinfo(ConventionID) -- 00:02:20 

-- ceci fait à aprtir des donnée de la prod pour ajuster des date de RIN estimé : ----> juste pour rel 2016
UPDATE r set 
	 r.DateRIEstimé = rc.DateRIEstimé
	 ,r.ApresEcheance = rc.ApresEcheance
from ConventionRCinfo r
join [univbase_pcee].dbo.ConventionRCinfo rc on r.ConventionNo = rc.ConventionNo
where	
	(r.DateRIEstimé <> rc.DateRIEstimé
	or r.ApresEcheance <> rc.ApresEcheance
	)
	AND r.ConventionNo not in (
		select c.ConventionNo
		from [univbase_pcee].dbo.Un_Unit u
		join [univbase_pcee].dbo.Un_Convention c on u.ConventionID = c.ConventionID
		GROUP by c.ConventionNo
		HAVING max(u.dtFirstDeposit) > '2016-12-31'
		)



exec psCONV_ReleveDeCompteDatasetGlobal 'EtatPAEs' -- 00:00:20 --279546 -- drop table EtatPAEs


-----------------------------					
					
					
exec psCONV_ReleveDeCompteDatasetGlobal '20-DTL-1' -- 00:00:37 fait
exec psCONV_ReleveDeCompteDatasetGlobal '20-DTL-2' -- 00:09:17 fait
exec psCONV_ReleveDeCompteDatasetGlobal '20-DTL-5' -- 00:09:17 fait  --> NEW PRA

exec psCONV_ReleveDeCompteDatasetGlobal '10-SLD-1' -- 00:00:09 fait
exec psCONV_ReleveDeCompteDatasetGlobal '10-SLD-2' -- 00:00:04 fait
exec psCONV_ReleveDeCompteDatasetGlobal '10-SLD-3' -- 00:00:10 fait
exec psCONV_ReleveDeCompteDatasetGlobal '10-SLD-4' -- 00:00:20 fait
exec psCONV_ReleveDeCompteDatasetGlobal '10-SLD-5' -- 00:00:32 fait

exec psCONV_ReleveDeCompteDatasetGlobal '15-SLD-1' -- 00:00:02 fait
exec psCONV_ReleveDeCompteDatasetGlobal '15-SLD-2' -- 00:00:10 fait
--ici

exec psCONV_ReleveDeCompteDatasetGlobal '30-SLD-1' -- 00:00:10 fait

exec psCONV_ReleveDeCompteDatasetGlobal '30-SLD-4' -- 00:00:54 fait

exec psCONV_ReleveDeCompteDatasetGlobal '25-FRS-1' -- 5 sec fait
exec psCONV_ReleveDeCompteDatasetGlobal '25-FRS-2' -- 2 sec fait
exec psCONV_ReleveDeCompteDatasetGlobal '25-FRS-3' -- 2 sec fait
-- ici

exec psCONV_ReleveDeCompteDatasetGlobal '20-DTL-3' -- 00:46:00 fait
-- rendu ici
exec psCONV_ReleveDeCompteDatasetGlobal '30-SLD-2' -- 01:17:59 fait
-- rendu ici 2017-11-14
exec psCONV_ReleveDeCompteDatasetGlobal '20-DTL-4' -- 05:40:47 (9263068 row(s) affected -- voir pour créer dabord clustered index
exec psCONV_ReleveDeCompteDatasetGlobal '30-SLD-3' -- 07:10:15-- 257084 -- voir pour créer dabord clustered index
-- 07:10:12

create clustered index ClsIndex_tmpReleveCompteDTL_ConventionID on tmpReleveCompteDTL(ConventionID) -- 00:02:20



--ici

-- générer les projection de subvention et rendement
exec psCONV_ProjectionSubventionsEtLeurRendement_All 11, NULL, NULL

exec psCONV_ReleveDeCompteDatasetGlobal 'ProjParConvention' -- 00:00:08 -- drop table ProjParConvention select count(*) from ProjParConvention
create clustered index ClsIndex_tmpReleveCompte_ProjParConvention on ProjParConvention(ConventionID) 

exec psCONV_ReleveDeCompteDatasetGlobal 'tmpReleveCompte' -- final  00:08:57--7510751 -- drop table tmpReleveCompte

create clustered index ClsIndex_tmpReleveCompte_SubscriberID on tmpReleveCompte(SubscriberID) -- 00:05:54
...
CREATE PROCEDURE [dbo].[psCONV_ReleveDeCompteDatasetGlobal] (
		@WhatToInsert varchar(50)
...
declare
	...
	@dtDateTo datetime = '2017-12-31' -------------- METTRE À JOUR

	-- Pour projection des I BEC qui ne sorte pas de la ps de projection présentement
	,@TauxAnnuelComposeMensuellement FLOAT = 0.040741543  --Taux d'intérêt annuel composé mensuellement pour les I BEC
	...
	if not exists (select 1 from sysobjects where name = 'tmpReleveCompteDTL')
		CREATE TABLE [dbo].[tmpReleveCompteDTL]( -- drop table tmpReleveCompteDTL
			WhatToInsert varchar(30) NOT NULL,
			[TypeLigne] varchar(30) NOT NULL,
			[ConventionID] int NOT NULL,
			[QteUnite] [float] NOT NULL,
			[OperID] int NULL,
			[Correction] varchar(30) NULL,
			[OperTypeID1] varchar(15) NULL,
			[OperTypeID] varchar(15) NULL,
			[OperDate] datetime NULL,
			[Epargne] [MoMoney] NOT NULL,
			[Frais] [MoMoney] NOT NULL,
			[AssuranceSousc] [MoMoney] NULL,
			[AutreFrais] [MoMoney] NOT NULL,
			[SCEE] [MoMoney] NOT NULL,
			[BEC] [MoMoney] NOT NULL,
			[IQEE] [MoMoney] NOT NULL,
			[Revenu] [MoMoney] NOT NULL,
			[ComptePAE] [MoMoney] NOT NULL
		) ON [PRIMARY]
...
if @WhatToInsert = 'tmpReleveCompteAllConv'
begin

	if exists (select 1 from sysobjects where name = 'tmpReleveCompteAllConv')
		drop table tmpReleveCompteAllConv


	select DISTINCT c.ConventionID,ConventionStateIDFin = cssFin.ConventionStateID,AnneeQualification = NULL
	into tmpReleveCompteAllConv -- drop table tmpReleveCompteAllConv
	...
	where e.conventionno is null -- exclure les exclusion

	create index ind1 on tmpReleveCompteAllConv(conventionid)
END	



	if @WhatToInsert = 'ProjParConvention'
	begin	
...
-- Faire un aggégat par convention pour les valeurs projetées qui sont utilisées dans la relevé de compte.
	...
	-- Patch pour les I BEC qui ne sortent pas dans la ps psCONV_ProjectionSubventionsEtLeurRendement_All
	INSERT INTO ProjParConvention
	SELECT 
		c.ConventionID,
	...
	END --if @WhatToInsert = 'ProjParConvention'
	
	
	
		
if @WhatToInsert = 'AnneeQualification'
begin		
	...
	from tmpReleveCompteAllConv RC
	JOIN(
		SELECT 
			s.ConventionID
		...
		JOIN tmpReleveCompteAllConv rc ON S.ConventionID = rc.ConventionID
...
END
--select * from tmpReleveCompteAllConv
...
if @WhatToInsert = 'ConventionRCinfo'
begin
	...
	SELECT
		...
		,Cohorte =	CASE WHEN rc.AnneeQualification IS NULL and c.PlanID <> 4 THEN cast(c.YearQualif as varchar(4)) 
		...
		-- 2017-02-16 : PRENDRE SEULEMENT LES DATE DE RIN ESTIMÉ DES GROUPE D'UNTIÉ NON RÉSILIÉ
		,DateRIEstimé = MIN ( CASE WHEN ISNULL(u.TerminatedDate,'9999-12-31') <= @dtDateTo THEN NULL ELSE dbo.FN_UN_EstimatedIntReimbDate (PmtByYearID,PmtQty,BenefAgeOnBegining,InForceDate,IntReimbAge,IntReimbDateAdjust) END) 
		,DateRIN = MIN(u.IntReimbDate)
		,ApresEcheance =	case -- 2017-02-16 : PRENDRE SEULEMENT LES DATE DE RIN ESTIMÉ DES GROUPE D'UNTIÉ NON RÉSILIÉ
							when MIN ( CASE WHEN ISNULL(u.TerminatedDate,'9999-12-31') <= @dtDateTo THEN NULL ELSE dbo.FN_UN_EstimatedIntReimbDate (PmtByYearID,PmtQty,BenefAgeOnBegining,InForceDate,IntReimbAge,IntReimbDateAdjust) END) <= @dtDateTo --DateRIEstimé estimé est passé
								then 1 
							else 0 
							end
	...
	FROM 
		Un_Convention C
		JOIN tmpReleveCompteAllConv rc ON c.ConventionID = rc.ConventionID
		...
		LEFT JOIN (
			...
			FROM 
				...
				JOIN tmpReleveCompteAllConv rc ON c1.ConventionID = rc.ConventionID
		...
		LEFT JOIN
			...
			JOIN tmpReleveCompteAllConv rc ON CB.iID_Convention = rc.ConventionID
		...
		LEFT JOIN (
			...
			JOIN (
				...
				JOIN tmpReleveCompteAllConv RC ON RC.ConventionID = C1.ConventionID 
		...
		LEFT join (
			...
			from Un_Convention c
			join tmpReleveCompteAllConv RC ON RC.ConventionID = C.ConventionID 
			...
			LEFT JOIN (
				...
				FROM 
					...
					JOIN (
						...
						from Un_Unit u2
						JOIN tmpReleveCompteAllConv R2 ON R2.ConventionID = U2.ConventionID
						group by U2.ConventionID
						)TU	 on TU.ConventionID = S.ConventionID
					JOIN tmpReleveCompteAllConv RC ON RC.ConventionID = S.ConventionID
					JOIN (
						SELECT S1.ScholarshipID, MAXOperDate = MAX(O1.OperDate)
						FROM Un_Scholarship S1
						JOIN tmpReleveCompteAllConv RC ON RC.ConventionID = S1.ConventionID
	...
	--create index #indconv ON tmpReleveCompteAllConvinfo (ConventionID)


/************************************************
		État des PAE Payé et ou calculé
*************************************************/
/*
-- 2018-01-01 : N'est plus utile depuis le projet d'enlevement des critères d'admission au bourse
if @WhatToInsert = 'EtatPAEs'
begin
	select 
		c.ConventionID
			...
			from (
					SELECT 
						...
						Calcul = round(--2016-02-23
										dbo.fnCONV_ObtenirValeurUnitaireCohorte (C.PlanID, ISNULL(C.iAnnee_QualifPremierPAE, 0), S.ScholarshipNo) * rci.QteUnitesConverties --2016-02-19  --qu.UnitQty --2016-02-12
									,2)
					FROM Un_Scholarship S
...
END
*/
	--select * from tmpReleveCompteAllConvinfo
	--return		
	...
	INTO #DateSousc
	FROM Un_Convention c
	JOIN tmpReleveCompteAllConv c1 ON c.ConventionID = c1.ConventionID
	JOIN Un_Unit u ON u.ConventionID = c.ConventionID
	...
	INTO #DateSousc
	FROM Un_Convention c
	JOIN tmpReleveCompteAllConvinfo c1 ON c.ConventionID = c1.ConventionID
	JOIN Un_Unit u ON u.ConventionID = c.ConventionID
	...
	*/
	...
	INSERT INTO #TableRES (
		...
		SELECT
			...
			ur.UnitQty
		FROM Un_Unit U
		JOIN tmpReleveCompteAllConv c1 ON U.ConventionID = c1.ConventionID
		JOIN Un_Cotisation Ct ON Ct.UnitID = U.UnitID
		JOIN Un_Oper O ON O.OperID = Ct.OperID
	...
	if @WhatToInsert = '20-DTL-1'
	BEGIN
		insert into tmpReleveCompteDTL
		SELECT 
			WhatToInsert = '20-DTL-1',
			TypeLigne = '20-DTL',
			c.ConventionID,
	...
	if @WhatToInsert = '20-DTL-2'
	BEGIN
		insert into tmpReleveCompteDTL
		SELECT
			WhatToInsert = '20-DTL-2',
			TypeLigne = '20-DTL',
			U.ConventionID,
			...
			OperTypeID1 = CASE 
						...
						WHEN  ISNULL(rio.iID_Convention_Source,0) = u.ConventionID THEN rio.OperTypeID + '_NEG'
						WHEN  ISNULL(rio.iID_Convention_Destination,0) = u.ConventionID THEN rio.OperTypeID + '_POS'
	...
	if @WhatToInsert = '20-DTL-3'
	BEGIN
		insert into tmpReleveCompteDTL
		SELECT 
			WhatToInsert = '20-DTL-3',
			TypeLigne = '20-DTL',
			ce.conventionid,
			QteUnite = 0,
			o.OperID,
			...
			OperTypeID1 = CASE 
						...
						WHEN  ISNULL(rio.iID_Convention_Source,'') = ce.ConventionID THEN rio.OperTypeID + '_NEG'
						WHEN  ISNULL(rio.iID_Convention_Destination,'') = ce.ConventionID THEN rio.OperTypeID + '_POS'
						ELSE  o.OperTypeID
						end,
			o.OperTypeID,
			o.OperDate,
		...
		JOIN tmpReleveCompteAllConv c1 ON ce.ConventionID = c1.ConventionID
		LEFT JOIN Un_TIO tio ON o.OperID = tio.iOUTOperID OR o.OperID = tio.iTINOperID OR o.OperID = tio.iTFROperID
		LEFT JOIN #oCancel oC ON o.OperID = oC.OperID OR o.OperID = oC.OperIDCancel
		LEFT JOIN #oCorrection Cor ON Cor.OperIDCorrect = o.OperID
		LEFT JOIN tblOPER_OperationsRIO rio ON o.OperID = rio.iID_Oper_RIO
	...
	if @WhatToInsert = '20-DTL-4'
	BEGIN
		insert into tmpReleveCompteDTL
		SELECT DISTINCT
			WhatToInsert = '20-DTL-4',
			TypeLigne = '20-DTL',
			co.ConventionID,
			QteUnite = 0,
			o.OperID,
			...
			SCEE = 0,
			BEC = 0,
			IQEE = SUM(CASE WHEN  CharIndex(CO.ConventionOperTypeID, 'CBQ,MMQ', 1) > 0 /*co.ConventionOperTypeID in ('CBQ','MMQ')*/ THEN co.ConventionOperAmount ELSE  0 END),
			Revenu = SUM(CASE WHEN CharIndex(CO.ConventionOperTypeID, 'IBC,ICQ,III,IIQ,IMQ,INS,IS+,IST,INM,ITR,MIM,IQI', 1) > 0 /*co.ConventionOperTypeID in ('IBC','ICQ','III','IIQ','IMQ','INS','IS+','IST','INM','ITR','MIM','IQI')*/ THEN co.ConventionOperAmount ELSE  0 END),
			ComptePAE = SUM(CASE WHEN CharIndex(CO.ConventionOperTypeID, 'BRS,AVC,RTN', 1) > 0 /* co.ConventionOperTypeID in ('BRS','AVC','RTN')*/ THEN co.ConventionOperAmount ELSE  0 END)
		FROM 
			un_oper o
			JOIN Un_ConventionOper co ON co.operID = o.OperID
			JOIN tmpReleveCompteAllConv c1 ON co.ConventionID = c1.ConventionID
			LEFT JOIN #oCancel oC ON o.OperID = oC.OperID OR o.OperID = oC.OperIDCancel
			LEFT JOIN Un_TIO tio ON o.OperID = tio.iOUTOperID OR o.OperID = tio.iTINOperID OR o.OperID = tio.iTFROperID
			LEFT JOIN #oCorrection Cor ON Cor.OperIDCorrect = o.OperID
			LEFT JOIN tblOPER_OperationsRIO rio ON o.OperID = rio.iID_Oper_RIO
	...
	if @WhatToInsert = '20-DTL-5'
	BEGIN
		--print 'ici'
		insert into tmpReleveCompteDTL
		-- section pour les PRA et leur retenues d'impôt
		SELECT DISTINCT
			WhatToInsert = '20-DTL-5',
			TypeLigne = '20-DTL',
			co.ConventionID,
			QteUnite = 0,
			o.OperID,
	...
	if @WhatToInsert = '10-SLD-2'
	BEGIN
		insert into tmpReleveCompteDTL
		SELECT
			WhatToInsert = '10-SLD-2',
			TypeLigne = '10-SLD',
			U.ConventionID,
	...
	if @WhatToInsert = '10-SLD-3'
	BEGIN
		insert into tmpReleveCompteDTL
		SELECT
			WhatToInsert = '10-SLD-3',
			TypeLigne = '10-SLD',
			U.ConventionID,
			QteUnite = 0,
			OperID = NULL,
	...
	if @WhatToInsert = '15-SLD-2'
	BEGIN
		insert into tmpReleveCompteDTL
		SELECT
			WhatToInsert = '15-SLD-2',
			TypeLigne = '15-SLD',
			U.ConventionID,
			QteUnite = 0,
			OperID = NULL,
	...
	if @WhatToInsert = '10-SLD-4'
	BEGIN	
		insert into tmpReleveCompteDTL
		SELECT 
			WhatToInsert = '10-SLD-4',
			TypeLigne = '10-SLD',
			ce.conventionid,
			QteUnite = 0,
			OperID = NULL,
	...
	if @WhatToInsert = '10-SLD-5'
	BEGIN	
		insert into tmpReleveCompteDTL
		SELECT --DISTINCT
			WhatToInsert = '10-SLD-5',
			TypeLigne = '10-SLD',
			c.ConventionID,
			QteUnite = 0,
			OperID = NULL,
	...
	END
			...
			--------------------------------------- solde à la fin ---------------------------
	if @WhatToInsert = '30-SLD-1'
	BEGIN	
		insert into tmpReleveCompteDTL
		SELECT
			WhatToInsert = '30-SLD-1',
			TypeLigne = '30-SLD',
			U.ConventionID,
	...
	if @WhatToInsert = '30-SLD-2'
	BEGIN	
		insert into tmpReleveCompteDTL
		SELECT
			WhatToInsert = '30-SLD-2',
			TypeLigne = '30-SLD',
			U.ConventionID,
			QteUnite = 0,
			OperID = NULL,
	...
	if @WhatToInsert = '30-SLD-3'
	BEGIN	
		insert into tmpReleveCompteDTL
		SELECT 
			WhatToInsert = '30-SLD-3',
			TypeLigne = '30-SLD',
			ce.conventionid,
			QteUnite = 0,
			OperID = NULL,
	...
	if @WhatToInsert = '30-SLD-4'
	BEGIN	
		insert into tmpReleveCompteDTL
		SELECT DISTINCT
			WhatToInsert = '30-SLD-4',
			TypeLigne = '30-SLD',
			co.ConventionID,
			QteUnite = 0,
			OperID = NULL,
	...
	if @WhatToInsert = '25-FRS-1'
	BEGIN	
		insert into tmpReleveCompteDTL
		SELECT
			WhatToInsert = '25-FRS-1',
			TypeLigne = '25-FRS',
			U.ConventionID,
			QteUnite = 0,
			OperID = NULL,
	...
	if @WhatToInsert = '25-FRS-2'
	BEGIN	
		insert into tmpReleveCompteDTL
		SELECT 
			WhatToInsert = '25-FRS-2',
			TypeLigne = '25-FRS',
			c.ConventionID,
			QteUnite = 0,
			OperID = NULL,
	...
	if @WhatToInsert = '25-FRS-3'
	BEGIN	
		insert into tmpReleveCompteDTL
		-- Un Moins, un solde à 0 par convention
		SELECT 
			WhatToInsert = '25-FRS-3',
			TypeLigne = '25-FRS',
			c1.ConventionID,
			QteUnite = 0,
			OperID = NULL,
...
if @WhatToInsert = 'tmpReleveCompte'
begin
	...
	FROM tmpReleveCompteDTL T1
	JOIN ConventionRCinfo /*tmpReleveCompteAllConvinfo*/ ci ON ci.ConventionID = T1.ConventionID
