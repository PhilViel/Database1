...
Note                :	2010-02-02	Donald Huppé	Création
						...
						2018-01-29	Donald Huppé	Ajout du champ RST (ristourne) et retirer RST du compte BRS
exec RP_UN_ScholarshipAccounting_Oper '2016-01-01','2016-12-31' , 'B'


RP_UN_ScholarshipAccounting_Oper	'2018-01-01', '2018-01-25', 'B'
RP_UN_ScholarshipAccounting_oPER	'2018-01-01', '2018-01-25', 'B'
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[RP_UN_ScholarshipAccounting_Oper] (
...
BEGIN
	...
	FROM (
			...
			SELECT
				...
				fRST = SUM(case when PAEOperID <> 0 then RST else 0 end),
				...
				fTotal = 
					...
					SUM(case when PAEOperID <> 0 then RST else 0 end),
			...
			FROM ( 
				...
				SELECT
					...
					RST = CASE WHEN ConventionOperTypeID = 'RST' THEN -ConventionOperAmount ELSE 0 END	
