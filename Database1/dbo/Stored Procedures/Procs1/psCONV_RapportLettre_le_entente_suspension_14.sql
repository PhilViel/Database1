...
Code du service		: psCONV_RapportLettre_le_entente_suspension_14
Nom du service		: Générer la lettre : le_entente_suspension_14
But 				: 
Facette				: CONV
...
Exemple d’appel		:	
						EXEC psCONV_RapportLettre_le_entente_suspension_14 421480
...
****************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportLettre_le_entente_suspension_14] @humanID integer
AS
BEGIN
	DECLARE
		@today datetime,
	...
	SELECT distinct
		...
		PlanClassification = [dbo].[fnCONV_ObtenirDossierClient](c.SubscriberID,1) + '\' + replace(LEFT(CONVERT(VARCHAR, @today, 120), 10),'-','')
							+ case HS.langID when 'FRA' then '_le_entente_suspension_14' when 'ENU' then '_le_entente_suspension_14_ang' end,
