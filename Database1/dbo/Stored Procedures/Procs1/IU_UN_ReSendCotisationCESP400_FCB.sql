...
Nom 					:	IU_UN_ReSendCotisationCESP400_FCB 
...
Exemple d'appel		:
					EXECUTE IU_UN_ReSendCotisationCESP400_FCB 355817,23218094,20241358,1,NULL
...
Note				:	
	...
	ADX0001289	UP	2008-03-14	Bruno Lapointe			Problème de gestion de dtRegStartDate avec heures,
														minutes et/ou secondes.
...
*************************************************************************************************/
CREATE PROCEDURE [dbo].[IU_UN_ReSendCotisationCESP400_FCB] 
	(
...
BEGIN
	...
	insert INTO @tCotToReSend VALUES (	@iCotisationID,@iOperID,'FCB')
	...
	IF @iCotisationBlobID > 0
		INSERT INTO @tCotToReSend
			...
			WHERE OC.OperID IS NULL -- Exclu les annulations et les annulées
				...
				AND( O.OperTypeID <> 'CPA' -- Exclu les CPA anticipés
...
*/
	...
	EXECUTE @iResult = IU_UN_CancelCotisationCESP400_FCB @iCotisationID, @iOperID, 1, @iIDConnect, @bSansVerificationPCEE400

	IF @iResult > 0 AND @iCotisationID > 0
	BEGIN
		...
		INSERT INTO Un_CESP400 (
			...
			WHERE C.dtRegStartDate IS NOT NULL
				...
				AND( Ct.EffectDate > dbo.FN_CRQ_DateNoTime(C.dtRegStartDate)
					OR ( Ct.EffectDate = dbo.FN_CRQ_DateNoTime(C.dtRegStartDate)
						AND Ct.OperID >= ISNULL(FCB.FCBOperID,Ct.OperID-1)
						)
					)
				AND OT.GovernmentTransTypeID = 11
				AND( O.OperTypeID <> 'TFR' -- Si c'est un TFR il doit être marqué à envoyer au PCEE
	...
	DECLARE crCotToReSend CURSOR FOR
		SELECT 
			Ct.CotisationID,
			Ct.OperID,
			OT.OperTypeID
	...
	BEGIN
		...
		BEGIN
			...
			OR EXISTS ( -- Vérifie que c'est la dernière opération de type RES de la convention.
				...
				JOIN Un_Oper O2 ON O2.OperID = Ct2.OperID AND O2.OperTypeID = 'RES' AND O2.OperDate > O.OperDate
				WHERE Ct.CotisationID = @CotisationID
				)
		...
		BEGIN
			-- Insère les enregistrements 400 de type 19
			EXECUTE @iResult = IU_UN_CESP400ForCotisation 1, @CotisationID, 19, 0
		END
		ELSE IF @OperTypeID = 'TRA'
		BEGIN
			IF EXISTS	( -- Vérifie s'il s'agit d'une cotisation augmentant le solde des épargnes et frais
							SELECT *
							FROM Un_Cotisation
	...
	DECLARE crOperToReSend CURSOR FOR
		SELECT 
			OperID,
			OperTypeID
		FROM @tOperToReSend 
	...
	BEGIN
		IF @OperTypeID = 'PAE'
			...
			EXECUTE @iResult = IU_UN_CESP400ForOper 1, @OperID, 13, 0
