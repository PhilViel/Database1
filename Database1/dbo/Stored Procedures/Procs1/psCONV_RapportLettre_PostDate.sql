...
Code du service		: psConv_RapportLettre_PostDate
Nom du service		: Générer la lettre post daté
But 				: 
Facette				: CONV
...
Exemple d’appel		:	EXEC psConv_RapportLettre_PostDate '2037998,U-20011219005,U-20011219006'
						EXEC psConv_RapportLettre_PostDate 'R-20050225003,R-20050225005'
						EXEC psConv_RapportLettre_PostDate '2037998'
...
****************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportLettre_PostDate] @conv varchar(max)
AS
BEGIN
	DECLARE
	...
	SELECT distinct
		...
		PlanClassification = [dbo].[fnCONV_ObtenirDossierClient](c.subscriberID,1) + '\' + replace(LEFT(CONVERT(VARCHAR, @today, 120), 10),'-','') + '_le_acc_pd_' + @premierConvention 
