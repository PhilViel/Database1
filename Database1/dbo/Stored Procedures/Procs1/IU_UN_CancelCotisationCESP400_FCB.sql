...
drop proc IU_UN_CancelCotisationCESP400_FCB					
					
*************************************************************************************************/
CREATE PROCEDURE [dbo].[IU_UN_CancelCotisationCESP400_FCB] 
	(
		@iCotisationID	INT,	-- ID du blob qui contient les CotisationID des cotisations dont il faut annuler l’envoi au PCEE. 0 = aucun
		@iOperID		INT,	-- ID du blob qui contient les OperID des opérations dont il faut annuler l’envoi au PCEE. On doit y retrouver seulement les lignes pour lesquelles nous n’Avons pas de CotisationID (nulle) par exemple les PAE et les AVC. 0 = aucun
...
BEGIN
	...
	IF @iCotisationID > 0
		INSERT INTO @tCotToCancel VALUES (@iCotisationID)
		/*
			SELECT DISTINCT iVal 
			FROM dbo.FN_CRI_BlobToIntegerTable(@iCotisationBlobID)
		*/
	IF @iOperID > 0
		INSERT INTO @tOperToCancel VALUES (@iOperID)
		/*
			SELECT DISTINCT iVal 
...
*/
	...
	IF @iCotisationID > 0 AND @iResult > 0
	BEGIN
		INSERT INTO Un_CESP400 (
			...
			SELECT
				...
				C4.tiPCGType,
				CASE 
					WHEN C4.tiCESP400TypeID IN(11,24) THEN -ISNULL(SUM(C9.fCLB),0)
				ELSE -C4.fCLB -- ici
				END,
				-C4.fEAPCLB,
				-C4.fPG,
