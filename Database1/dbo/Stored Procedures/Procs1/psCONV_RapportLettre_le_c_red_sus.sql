...
Code du service		: psCONV_RapportLettre_le_c_red_sus
...
Exemple d’appel		:	
			EXEC psCONV_RapportLettre_le_c_red_sus 'X-20101203018,U-20110418005'
			EXEC psCONV_RapportLettre_le_c_red_sus	'U-20050419032,U-20050222033,U-20050419034,U-20050222034'
drop procedure psCONV_RapportLettre_le_c_red_sus
...
Historique des modifications:
		...
		------------	----------------------------------	-----------------------------------------	------------
		2014-08-11		Maxime Martel						Création du service	
		
****************************************************************************************************/
CREATE PROCEDURE [dbo].[psCONV_RapportLettre_le_c_red_sus] @cConventionno varchar(max)
AS
BEGIN
	DECLARE
		@today datetime,
	...
	SELECT distinct
		...
		PlanClassification = [dbo].[fnCONV_ObtenirDossierClient](c.SubscriberID,1) + '\' + replace(LEFT(CONVERT(VARCHAR, @today, 120), 10),'-','')
							+ case HS.langID when 'FRA' then '_le_c_red_sus' when 'ENU' then '_le_c_red_sus_ang' end
