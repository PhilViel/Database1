...
Note                :	2012-11-29	Donald Huppé	Créaton à partir de GU_RP_VentesRepConcoursPresident_2013
						2013-03-15	GLPI 9294
						2013-04-30	Donald Huppé	GLPI 9568 : ajuster le rmeplacement de "agence Maryse Logelin" par "agence Nouveau-Brunswick"
						2013-08-06	Donald Huppé	GLPI 10015	: il faut vider la table #GrossANDNetUnits avant de faire un insert dedans.
						2013-11-12	Donald Huppé	glpi 10514 - réattribution des rep dans certaines agences
*********************************************************************************************************************/

--  exec psREPR_RapportMortier_2014 '2012-10-08', '2013-10-03' , '2011-10-10',  '2012-10-03'

CREATE procedure [dbo].[psREPR_RapportMortier_2014] 
	(
...
create table #VenteDirPrec (
	...
	group by 
		...
		WHERE 
			G.Recrue = 1 
			AND Rep.RepCode NOT IN ('7794','7823','7782','7715') --Nicole Laflamme, Vénus Fréchette, Kathleen Fauteux et Isabelle Danis-Marineau
	...
	WHERE 
		G.Recrue = 1 
		AND Rep.RepCode NOT IN ('7794','7823','7782') --Vénus Fréchette (Ghislain Thibeault), Kathleen Fauteux (Sophie Babeux), Isabelle Danis-Marineau (Michel Maheu).
		and businessStart between dateADD(month,-12,@StartDate) and cast(cast(year(@StartDate)+1 as varchar(4)) + '-09-30' as datetime) 
	...
	order by REPLACE(Agence,'Agence','Ag.')--,F.Groupe, F.RepID
