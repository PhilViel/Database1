...
EXEC psREPR_RapportDeVenteEtCommission_Sommaire
...
EXEC psREPR_RapportDeVenteEtCommission_Sommaire
					@StartDate = '2017-10-16', -- Date de début
					@EndDate = '2017-10-22', -- Date de fin
					@RepID = 149593 --691759 -- -- -- --691759 --  -- --476221 --416305 -- ID du représentant
					,@Type = 'REP'
					

EXEC psREPR_RapportDeVenteEtCommission_Sommaire
					@StartDate = '2017-09-11', -- Date de début
					@EndDate = '2017-09-17', -- Date de fin
					@RepID = 517345 --691759 -- --476221 --416305 -- ID du représentant
					,@Type = 'REP'


EXEC psREPR_RapportDeVenteEtCommission_Sommaire
					@StartDate = '2018-10-31', -- Date de début
					@EndDate = '2018-11-24', -- Date de fin
					@RepID = 149497 -- ID du représentant
					,@Type = 'REP'
 ...
 ****************************************************************************************************/
CREATE PROCEDURE [dbo].[psREPR_RapportDeVenteEtCommission_Sommaire]
	(	
	@StartDate		DATETIME,
	@EndDate		DATETIME,
	@RepID			INT,
...
-------------------------------------------   2  FIN  ----------------------------------------------------
	...
	-- Commission I BEC versé

	INSERT INTO #Dataset_Sommaire
	SELECT
		Sort = 235,   
		GroupeID =  3, 
		LaDescription = CASE WHEN @vcLangID = 'ND' THEN '' ELSE  'Boni BEC versé' END,  
		Valeur = SUM(CB.mMontant_ComBEC),
		ValeurTXT = ''
		,TypeValeur = 'C2'
		,CouleurDETAIL = '#d4effc' --BLEU PALE
	FROM tblREPR_CommissionsBEC CB
	JOIN Un_RepTreatment RT ON RT.RepTreatmentID = CB.RepTreatmentID
	WHERE CB.RepID = @RepID
		AND RT.RepTreatmentDate BETWEEN @StartDate AND @EndDate
		AND CB.mMontant_ComBEC >= 0 -- Les montants positifs sont les bonis versés
	HAVING ISNULL(SUM(CB.mMontant_ComBEC),0) <> 0


	-- Commission I BEC repris

	INSERT INTO #Dataset_Sommaire
	SELECT
		Sort = 236,   
		GroupeID =  3, 
		LaDescription = CASE WHEN @vcLangID = 'ND' THEN '' ELSE  'Boni BEC repris' END,  
		Valeur = SUM(CB.mMontant_ComBEC),
		ValeurTXT = ''
		,TypeValeur = 'C2'
		,CouleurDETAIL = '#d4effc' --BLEU PALE
	...
	WHERE CB.RepID = @RepID
		AND RT.RepTreatmentDate BETWEEN @StartDate AND @EndDate
		AND CB.mMontant_ComBEC < 0 -- Les montants négatif sont les reprises de boni
	HAVING ISNULL(SUM(CB.mMontant_ComBEC),0) <> 0
