...
Nom                 :	RP_UN_RepRookiesContest
Description         :	Procédure stockée du rapport : Concours des recrues
Valeurs de retours  :	Dataset 
Note                :	ADX0000430	IA	2004-09-07	Bruno Lapointe		Création (13.06)
								ADX0001433	BR	2005-05-24	Bruno Lapointe		Correction du bogue : les ventes des représentants
																							recrus dont la date de début d'affaire est avant
																							la date de début de l'interval doivent apparaîtrent.
								ADX0001206	IA	2007-01-09	Bruno Lapointe		Optimisation.
...
*********************************************************************************************************************/
CREATE PROCEDURE [dbo].[RP_UN_RepRookiesContest] (
...
BEGIN
	...
	SELECT
		...
		@EndDate = 
			CASE 
				WHEN EndDate <= 0 THEN NULL
			ELSE dbo.FN_CRQ_DateNoTime(EndDate) + 1
			END
	...
	CREATE TABLE #SalesByRookies (
		UnitID INTEGER PRIMARY KEY)

	CREATE TABLE #RepRookiesContest (
		RepID INTEGER,
		LastName VARCHAR(75),
	...
	INSERT INTO #SalesByRookies
		SELECT 
			UnitID
		FROM dbo.Un_Unit U
		JOIN Un_Rep R ON R.RepID = U.RepID
		WHERE (dbo.fn_Un_IsRecruit(R.BusinessStart, U.dtFirstDeposit) = 1) -- Ventes faites par une recrue
		  AND (U.dtFirstDeposit >= @StartDate) -- La vente doit être fait dans l'intervalle du concours
		  AND (U.dtFirstDeposit < @EndDate OR @EndDate IS NULL)
	...
	INSERT INTO #GrossUnits
		...
		FROM dbo.Un_Unit U 
		JOIN #SalesByRookies S ON S.UnitID = U.UnitID
		LEFT JOIN (
			SELECT 
				U1.UnitID,
	...
	INSERT INTO #TerminatedUnits
		...
		WHERE (UR.FeeSumByUnit < M.FeeByUnit) -- Compte uniquement les résiliations dont les frais n'étaient pas totalement couvert
		  AND (UR.ReductionDate >= @StartDate) -- La vente doit être fait dans l'intervalle du concours
		  AND (UR.ReductionDate < @EndDate OR @EndDate IS NULL)
	...
	INSERT INTO #MaxPctBoss
		...
		JOIN (
			...
			WHERE RepRoleID = 'DIR'
			  ...
			  AND (EndDate IS NULL OR @EndDate IS NULL OR EndDate+1 >= @EndDate)
			GROUP BY
				RepID
		...
		WHERE RB.RepRoleID = 'DIR'
		  ...
		  AND (RB.EndDate IS NULL OR @EndDate IS NULL OR RB.EndDate+1 >= @EndDate)
		GROUP BY
			RB.RepID

	-- Sélection finale
	INSERT INTO #RepRookiesContest
		SELECT
	...
	DROP TABLE #SalesByRookies
	DROP TABLE #RepRookiesContest
	...
	IF DATEDIFF(SECOND, @dtBegin, @dtEnd) > @siTraceReport
		...
		INSERT INTO Un_Trace (
			...
			SELECT
				...
				@dtBegin,
				@dtEnd,
				'Concours des recrues',
				'RP_UN_RepRookiesContest',
				'EXECUTE RP_UN_RepRookiesContest @ConnectID = '+CAST(@ConnectID AS VARCHAR)+
					', @RepContestCfgID = '+CAST(@RepContestCfgID AS VARCHAR)+
					', @DataOrder = '+@DataOrder
END

/*  Sequence de test - par: PLS - 09-05-2008
	exec [dbo].[RP_UN_RepRookiesContest] 
