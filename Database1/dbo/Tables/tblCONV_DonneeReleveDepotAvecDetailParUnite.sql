CREATE TABLE [dbo].[tblCONV_DonneeReleveDepotAvecDetailParUnite] (
...
GO
CREATE NONCLUSTERED INDEX [IX_CONV_DonneeReleveDepotAvecDetailParUnite_iIDConvention]
    ON [dbo].[tblCONV_DonneeReleveDepotAvecDetailParUnite]([iIDConvention] ASC)
    INCLUDE([vcTypeOperation], [mBec]) WITH (FILLFACTOR = 90);


GO
EXECUTE sp_addextendedproperty @name = N'MS_Description', @value = N'Table des données discriminés par groupe d`unité, servant à la génération des relevés de dépôt', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'TABLE', @level1name = N'tblCONV_DonneeReleveDepotAvecDetailParUnite';
