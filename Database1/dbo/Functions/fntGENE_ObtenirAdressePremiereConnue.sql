...
Code du service	: fntGENE_ObtenirPremiereAdresseConnue
Nom du service	: Déterminer quelle était la 1ère adresse
But 			: Retourner la 1ère adresse d’une personne ou d’une entreprise, dans le format abrégé ou non.
Facette			: GENE
...
Exemple d’appel	:   select * from dbo.fntGENE_ObtenirPremiereAdresseConnue(DEFAULT, DEFAULT, '2014-12-31', DEFAULT)
				    select * from dbo.fntGENE_ObtenirPremiereAdresseConnue(250597, DEFAULT, '2014-12-31', 1)
				    select * from dbo.fntGENE_ObtenirPremiereAdresseConnue(DEFAULT, 1, '2014-12-31', DEFAULT)

Historique des modifications:
        Date		Programmeur	                Description
        ----------  ------------------------    ------------------------------------------------------
        2017-11-25  Steeve Picard               Création du service basé sur la function «fntGENE_ObtenirAdresseEnDate_PourTous»
*********************************************************************************************************************/
CREATE FUNCTION dbo.fntGENE_ObtenirAdressePremiereConnue (
    @iID_Humain   INT,
    @iID_Type     INT      = 1,
...
AS RETURN
     ...
     FROM
     (
        SELECT X.*, Row_Num = ROW_NUMBER() OVER(PARTITION BY X.iID_Source, X.cType_Source, X.iID_Type ORDER BY X.dtDate_Debut)
          FROM (
             ...
             WHERE A.iID_Source = ISNULL(@iID_Humain, A.iID_Source)
                   ...
                   AND A.dtDate_Debut > ISNULL(@dtDate_Debut, '1950-01-01')
             UNION
             SELECT A.iID_Adresse, A.iID_Type, A.iID_Source, A.cType_Source, 
