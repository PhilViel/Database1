/****************************************************************************************************
Code de service    :   fnIQEE_CalculerSoldeMajoration_Convention
...
Exemple d'appel:
    ...
    SELECT dbo.fnIQEE_CalculerSoldeMajoration_Convention (@ID, '2017-12-31')
...
Historique des modifications :
    ...
    2012-12-13  Stéphane Barbeau            Ajout clause OR (O.OperTypeID = 'IQE' AND CO.ConventionOperAmount < 0.00)
    ...
    2013-12-13  Stéphane Barbeau            Requête @mMontant_IQEE_Majoration: Ajustement condition (O.OperTypeID <> 'IQE'  AND O.OperDate <= @dtDate_Fin)
    2018-02-08  Steeve Picard               Déplacement de «siAnnee_Fiscale» de la table «tblIQEE_Fichiers» vers les tables d'évèments
****************************************************************************************************/
CREATE FUNCTION dbo.fnIQEE_CalculerSoldeMajoration_Convention
(
    @iID_Convention INT,
    @dtDate_Fin DATETIME
)
...
BEGIN
    ...
    DECLARE @mTotal_IQEE_Majoree MONEY
    DECLARE @mMontant_Subsequent_T06_Erreur MONEY
    DECLARE @mMontant_Subsequent_T06_Reponses MONEY

    SELECT 
        @mMontant_IQEE_Majoration = ISNULL(IQEE.iqee, 0)
    FROM 
        dbo.Un_Convention C
        LEFT JOIN (
            SELECT
                iqee = ISNULL(SUM(ISNULL(CO.ConventionOperAmount, 0)), 0),
                CO.ConventionID
            FROM 
    ...
    WHERE 
        C.ConventionOperID IN (
            ...
            WHERE 
                ...
                AND UCO.ConventionOperTypeID = 'MMQ'
    ...
    WHERE 
        C.ConventionOperID IN (
            SELECT DIS.iID_Transaction_Convention_MMQ_Renversee
            FROM 
                dbo.tblIQEE_ImpotsSpeciaux DIS
                JOIN dbo.Un_ConventionOper UCO ON UCO.ConventionOperID = DIS.iID_Transaction_Convention_MMQ_Renversee
    ...
    WHERE 
        C.ConventionOperID IN (
            SELECT 
                RIS.iID_Paiement_Impot_MMQ
            FROM 
                dbo.tblIQEE_ImpotsSpeciaux DIS
                JOIN dbo.tblIQEE_ReponsesImpotsSpeciaux RIS ON DIS.iID_Impot_Special = RIS.iID_Impot_Special_IQEE
                JOIN dbo.Un_ConventionOper UCO ON UCO.ConventionOperID = RIS.iID_Paiement_Impot_MMQ
                JOIN dbo.Un_Oper UOP ON UOP.OperID = UCO.OperID
            WHERE 
                DIS.iID_Convention = @iID_Convention
                ...
                AND DIS.tiCode_Version IN ( 0, 2 )
                AND DIS.siAnnee_Fiscale <= YEAR(@dtDate_Fin)
                AND RIS.iID_Paiement_Impot_MMQ IS NOT NULL
                AND UCO.ConventionOperTypeID = 'MMQ'
                AND UOP.OperDate > @dtDate_Fin
        )

    SET @mTotal_IQEE_Majoree = @mMontant_IQEE_Majoration + @mMontant_Subsequent_ReponsesT02 + @mMontant_Subsequent_T06_Erreur + @mMontant_Subsequent_T06_Reponses

    RETURN @mTotal_IQEE_Majoree
END
