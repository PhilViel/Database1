...
*********************************************************************************************************************/
CREATE FUNCTION [dbo].[fntGENE_ObtenirPremiereAdresseConnue] (
    @iID_Humain   INT,
    @iID_Type     INT      = 1,
...
AS RETURN
     ...
     FROM
     (
        SELECT X.*, Row_Num = ROW_NUMBER() OVER(PARTITION BY X.iID_Source, X.cType_Source ORDER BY X.dtDate_Debut)
          FROM (
